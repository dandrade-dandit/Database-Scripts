...
/****** Object:  View [dbo].[gvw_resumo_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_da_det_agua_luz]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_da_det_despesas]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_da_det_despesas_receitas_teca]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
SELECT	(SELECT TOP 1 a.[cd_aerop] FROM GEACutcx.dbo.cor_aeroporto_u a WHERE a.[cd_aerop_num] = RECEITAS.dependencia) AS sigla_dep
     ...
     ,	DESPESAS.total_despesa_jan
	 ,	DESPESAS.total_despesa_fev
	 ,	DESPESAS.total_despesa_mar
	 ,	DESPESAS.total_despesa_abr
	 ,	DESPESAS.total_despesa_mai
	 ,	DESPESAS.total_despesa_jun
	 ,	DESPESAS.total_despesa_jul
	 ,	DESPESAS.total_despesa_ago
	 ,	DESPESAS.total_despesa_set
	 ,	DESPESAS.total_despesa_out
	 ,	DESPESAS.total_despesa_nov
	 ,	DESPESAS.total_despesa_dez
     ,	DESPESAS.total_despesa
     ,	RECEITAS.grupo_receita
     ,  RECEITAS.total_receita_jan
	 ,  RECEITAS.total_receita_fev
	 ,  RECEITAS.total_receita_mar
	 ,  RECEITAS.total_receita_abr
	 ,  RECEITAS.total_receita_mai
	 ,  RECEITAS.total_receita_jun
	 ,  RECEITAS.total_receita_jul
	 ,  RECEITAS.total_receita_ago
	 ,  RECEITAS.total_receita_set
	 ,  RECEITAS.total_receita_out
	 ,  RECEITAS.total_receita_nov
	 ,  RECEITAS.total_receita_dez
     ,  RECEITAS.total_receita
FROM (
	SELECT [dependencia]
		  ,'RECEITA' AS grupo_receita
		  ,[processing_yr] as ano
		  ,SUM([ldr_amt_1]) as total_receita_jan
		  ,SUM([ldr_amt_2]) as total_receita_fev
		  ,SUM([ldr_amt_3]) as total_receita_mar
		  ,SUM([ldr_amt_4]) as total_receita_abr
		  ,SUM([ldr_amt_5]) as total_receita_mai
		  ,SUM([ldr_amt_6]) as total_receita_jun
		  ,SUM([ldr_amt_7]) as total_receita_jul
		  ,SUM([ldr_amt_8])  as total_receita_ago
		  ,SUM([ldr_amt_9])  as total_receita_set
		  ,SUM([ldr_amt_10]) as total_receita_out
		  ,SUM([ldr_amt_11]) as total_receita_nov
		  ,SUM([ldr_amt_12]) as total_receita_dez
  ...
  SELECT   [dependencia]
		  ...
		  ,[processing_yr] as ano
		  ,SUM(jan) as total_despesa_jan
		  ,SUM(fev) as total_despesa_fev
		  ,SUM(mar) as total_despesa_mar
		  ,SUM(abr) as total_despesa_abr
		  ,SUM(mai) as total_despesa_mai
		  ,SUM(jun) as total_despesa_jun
		  ,SUM(jul) as total_despesa_jul
		  ,SUM(ago)  as total_despesa_ago
		  ,SUM(setemb)  as total_despesa_set
		  ,SUM(outub) as total_despesa_out
		  ,SUM(nov) as total_despesa_nov
		  ,SUM(dez) as total_despesa_dez
	  ...
	  FROM (
			SELECT [dependencia]
				  ...
				  ,SUM([ldr_amt_7]) as jul
				  ,SUM([ldr_amt_8])  as ago
				  ,SUM([ldr_amt_9])  as setemb
				  ,SUM([ldr_amt_10]) as outub
				  ,SUM([ldr_amt_11]) as nov
				  ,SUM([ldr_amt_12]) as dez
...
/****** Object:  View [dbo].[gvw_resumo_da_det_luz_custo]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
create view [dbo].[gvw_resumo_da_det_luz_custo] as
SELECT [dependencia]
      ,(SELECT TOP 1 a.[cd_aerop] FROM GEACutcx.dbo.cor_aeroporto_u a WHERE a.[cd_aerop_num] = [dependencia]) AS sigla_dep
      ,[conta]
      ,custo
...
/****** Object:  View [dbo].[gvw_resumo_det_conta_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_det_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_det_det_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_det_rank_2_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_det_rank_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
as
	SELECT	SIGLA_DEP
		 ...
		 ,	CASE RANK() OVER (PARTITION BY SIGLA_DEP, ANO ORDER BY SUM(TOTAL_REALIZADO_ANO) DESC)
		       ...
		       ELSE 6
		    END RANK_TOP5
		 ,	SUM(REALIZADO_JAN) AS JAN
		 ,	SUM(REALIZADO_FEV) AS FEV
		 ,	SUM(REALIZADO_MAR) AS AR
		 ,	SUM(REALIZADO_ABR) AS ABR
		 ,	SUM(REALIZADO_MAI) AS MAI
		 ,	SUM(REALIZADO_JUN) AS JUN
		 ,	SUM(REALIZADO_JUL) AS JUL
		 ,	SUM(REALIZADO_AGO) AS AGO
		 ,	SUM(REALIZADO_SET) AS "SET"
		 ,	SUM(REALIZADO_OUT) AS "OUT"
		 ,	SUM(REALIZADO_NOV) AS NOV
		 ,	SUM(REALIZADO_DEZ) AS DEZ    
		 ,	SUM(TOTAL_REALIZADO_ANO) AS SUM_REALIZADO_ANO
	  FROM	gvw_resumo_det_conta_da WITH (nolock) 
	 WHERE	GRUPO_CONTA = 'DESPESAS'
	 GROUP
...
/****** Object:  View [dbo].[gvw_resumo_det_rank_mes_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_det_rank_receitas_da]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  View [dbo].[gvw_resumo_det_rank_receitas_da_2]    Script Date: 12/11/2015 14:14:18 ******/
SET ANSI_NULLS ON
GO
