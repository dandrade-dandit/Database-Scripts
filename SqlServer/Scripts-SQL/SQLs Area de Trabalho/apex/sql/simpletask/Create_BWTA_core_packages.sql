...
BEGIN 
  ...
  WHEN v_command = c_cmdThread_STOP; 
    ...
    SYS.DBMS_LOCK.ALLOCATE_UNIQUE(c_lock_name_getProcToRun, v_lockhandle) ; 
    v_lockresult := SYS.DBMS_LOCK.REQUEST(v_lockhandle) ; 
    ...
    END IF; 
    COMMIT; 
    v_lockresult := SYS.DBMS_LOCK.RELEASE(v_lockhandle) ; 
    EXIT 
  WHEN v_seq IS NULL; 
