...
SELECT DISTINCT
 ...
 WHERE T.DATA >= TO_DATE('01/06/2011','DD/MM/YYYY')
 GROUP 
    BY VUI.MATRICULA
...
--CRIAÇÃO DOS ÍNDICES
create index idx_mv_dados_resumidos_01 on mv_dados_resumidos(ano) tablespace TSDRELPROXY;

create index idx_mv_dados_resumidos_02 on mv_dados_resumidos(mes) tablespace TSDRELPROXY;

create index idx_mv_dados_resumidos_03 on mv_dados_resumidos(lotacao) tablespace TSDRELPROXY;

create index idx_mv_dados_resumidos_04 on mv_dados_resumidos(dependencia) tablespace TSDRELPROXY;

create index idx_mv_dados_resumidos_05 on mv_dados_resumidos(matricula) tablespace TSDRELPROXY;
...
--CRIAÇÃO DOS ÍNDICES
create index idx_MV_DADOS_CONSOLIDADOS_01 on MV_DADOS_CONSOLIDADOS(MATRICULA, ANO, MES) tablespace TSDRELPROXY;
...
--CRIAÇÃO DOS ÍNDICES
create index idx_MV_DADOS_DETALHADOS_01 on MV_DADOS_DETALHADOS(MATRICULA, SITE, ANO, MES) tablespace TSDRELPROXY;
