CREATE OR REPLACE PACKAGE BME.pk_viagens  IS
...
--                        p_cod_viagem number := 0) return number;
...
--                      p_dt_ini out date,
--                      p_dt_fim out date);
-- function emp_ferias(p_mat in cadastros.emp_numero_matricula%type ) return varchar2;
...
-- pragma restrict_references(emp_ferias, WNDS, WNPS, RNPS);
END;
/


CREATE OR REPLACE PACKAGE BODY BME.pk_viagens IS 
-- function aprovador( p_dep_codigo number,
--                      p_uor_codigo number := 0, 
--                      p_emp_qfu_fun_codigo number := 0, 
--                      p_avi_ofu_codigo number, 
...
--   v_dep_dep_codigo dependencia.dep_dep_codigo%type; 
--   v_dep_tipo dependencia.dep_tipo%type; 
--  begin 
...
--                 end if; 
--           end if; 
--          else -- demais dependencia  sem ser a sede 
