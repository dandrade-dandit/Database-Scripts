   ...
   begin
       select count(username)
        ...
        and username = v_usuario;--verifica se a role passada como parametro é igual a uma das roles do SGBD
				...
				if v_RoleSistema > 0 then
						px_result_cod := 1;
    	  	  px_result_des := 'Erro de tentativa de bloqueio/desbloqueio de usuário(' || v_usuario ||') de banco de dados.';
    ...
    if (v_erro= 0 and px_flag = 0) then
        begin
            ...
            px_result_cod := 0;
    	  	  px_result_des := 'Usuário '||v_usuario|| ' desbloqueado.';
        exception
            when others then
            		px_result_cod := 1;
    	  	  	  px_result_des := 'Erro no bloqueio do usuário '||v_usuario;
                v_erro := 1;
        end;
    end if;
    ...
    if (v_erro= 0 and px_flag = 1) then
        begin
            ...
            EXECUTE IMMEDIATE v_sql;
            px_result_cod := 0;
    	  	  px_result_des := 'Usuário '||v_usuario|| ' bloqueado.';
        exception
            when others then
            	  px_result_cod := 1;
    	  	  	  px_result_des := 'Erro no desbloqueio do usuário '||v_usuario;
                v_erro := 1;
        end;
    end if;
    ...
    begin
        ...
        values
        (sysdate,'sp_idm_ativa_inativa_usuario','Sp para ativar e desativar usuário no banco de dados Oracle.',px_result_cod,px_result_des);
        commit;
    exception
        when  others then
