   ...
   begin
        ...
        and username = v_usuario;--verifica se a role passada como parametro é igual a uma das roles do SGBD
				...
				if v_RoleSistema > 0 then
						px_result_cod := 1;
    	  	  px_result_des := 'Erro de tentativa deleção de usuário(' || v_usuario ||') de banco de dados.';
    ...
    if (v_erro= 0) then
        begin
            ...
            px_result_cod := 0;
    	  	  px_result_des := 'Usuário '||v_usuario|| ' deletado com sucesso.';
        exception
            when others then
            		px_result_cod := 1;
    	  	  	  px_result_des := 'Usuário '||v_usuario|| ' não existe.';
                v_erro := 1;
        end;
    end if;
    ...
    begin
        ...
        values
        (sysdate,'sp_idm_del_usuario','Sp para alterar deletar usuário no banco de dados com perfil básico no Oracle.',px_result_cod,px_result_des);
        commit;
    exception
        when  others then
