--1
--Criar Roles, Usuários e grant role to usuários
DECLARE @DATABASE varchar(30)

set @DATABASE= 'ViagensWeb'
...
--2
--Gera os DDL para usuários ou roles
...
BEGIN
	...
	SET @sql = @sql + 'WHERE ([sysobjects].[name] not like ''dt%'' and [sysobjects].[name] not like ''sp%'' and [sysobjects].[name] not like ''fn%'') and [sysprotects].[uid] = @DatabaseUserID ' + CHAR(13) 
