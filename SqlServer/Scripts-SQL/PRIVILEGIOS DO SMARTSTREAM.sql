...
BEGIN
	...
	SET @sql = @sql + 'WHERE	name in (''AP_SISTEMA'') ' + CHAR(13) 
	...
	SET @sql2 = @sql2 + 'SELECT	''exec sp_addrolemember N'''''' + (select name from sysusers where issqlrole = 1 and uid = groupuid) + '''''', '''''' + (select name from sysusers where issqlrole = 0 and uid = memberuid) + ''''''''' + CHAR(13)
	SET @sql2 = @sql2 + 'FROM	sysmembers  ' + CHAR(13)
	SET @sql2 = @sql2 + 'WHERE   memberuid  in ( select uid  from sysusers where issqlrole = 0 and name = ''AP_SISTEMA'') ' + CHAR(13)
