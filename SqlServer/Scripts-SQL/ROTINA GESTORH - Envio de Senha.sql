declare
   pMatricula   number(7) := 2409922;
   pRetorno     varchar(200);
...
begin
	...
	EXCEPTION
   		WHEN OTHERS THEN
      		...
      		v_sql := 'ALTER USER '||v_usuario||' IDENTIFIED BY '||v_senha || ' ACCOUNT UNLOCK';
      		EXECUTE IMMEDIATE v_sql;
    END;
    ...
    if (v_senha_cpf = true) then --se a senha for composta pela CPF, informa na tela
    	...
    	----------------------------------------------------------------	
        envia_email('GestorH','denis_andrade@infraero.gov.br','Senha de acesso',v_corpo_msg);    
    end if;
