...
,		@IsOnlyPermissions tinyint

SET @IsOnlyPermissions = 1 --0 para tudo e 1 somente para as permissões
...
SELECT name
FROM	sysusers
WHERE	name not in ('guest','dbo','public','db_owner','db_accessadmin','db_securityadmin','db_ddladmin','db_backupoperator','db_datareader','db_datawriter','db_denydatareader','db_denydatawriter','Backup$Operators')
AND		issqlrole = 1
...
BEGIN
	...
	BEGIN
		...
		'--Created At: ' + CONVERT(varchar, GETDATE(), 103) + ' ' + CONVERT(varchar, GETDATE(), 108) + CHAR(13) +
		'--Created By: ' + ISNULL(SUSER_NAME(), 'sa') + CHAR(13) +
		'--Add Role To Database' + CHAR(13)

		IF	@IsOnlyPermissions = 0
		BEGIN
