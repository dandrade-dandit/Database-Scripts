          ...
          SELECT 'S' SIM
           ...
           WHERE NOT EXISTS
                      (SELECT 'S'
                        ...
                        WHERE CFA_OFU_CODIGO = OEM_OFU_CODIGO
                          AND CFA_NEW_DATA_INICIO = OEM_DATA_INICIO
                          AND CFA_NEW_FUN_CODIGO = OEM_FUN_CODIGO
                          AND CFA_EMP_NUMERO_MATRICULA = OEM_EMP_NUMERO_MATRICULA
                          AND NVL(CFA_FLAG_EXECUTO,'N') = 'N'
                       )
             AND OEM_FUN_CODIGO = C_FUN.NEW_FUN_CODIGO
          ...
          SELECT 'S' SIM
           ...
           WHERE NOT EXISTS
                      (SELECT 'S'
                        ...
                        WHERE CFA_OFU_CODIGO = OEM_OFU_CODIGO
                          AND CFA_NEW_DATA_INICIO = OEM_DATA_INICIO
                          AND CFA_NEW_FUN_CODIGO = OEM_FUN_CODIGO_ACUMULA
                          AND CFA_EMP_NUMERO_MATRICULA = OEM_EMP_NUMERO_MATRICULA
                          AND NVL(CFA_FLAG_EXECUTO,'N') = 'N'
                       )
             AND OEM_FUN_CODIGO_ACUMULA = C_FUN.NEW_FUN_CODIGO
          ...
          SELECT 'S' SIM
           ...
           WHERE NOT EXISTS
                      (SELECT 'S'
                        ...
                        WHERE CFA_OFU_CODIGO = OEM_OFU_CODIGO
                          AND CFA_NEW_DATA_INICIO = OEM_DATA_INICIO
                          AND CFA_NEW_FUN_CODIGO = OEM_FUN_CODIGO_SUBSTITUI
                          AND CFA_EMP_NUMERO_MATRICULA = OEM_EMP_NUMERO_MATRICULA
                          AND NVL(CFA_FLAG_EXECUTO,'N') = 'N'
                       )
             AND OEM_FUN_CODIGO_SUBSTITUI = C_FUN.NEW_FUN_CODIGO
      ...
      BEGIN
        ...
        LOOP
          BEGIN
            ...
            --
            IF C_FUN.TIP_OPERACAO = 'I' THEN -- Operação é 'I' (Insert)
              BEGIN
                --
                BEGIN  -- Procuro em atividades empregados o registro
                  ...
                  --
                EXCEPTION
                  WHEN NO_DATA_FOUND THEN -- Como não achou registro devo inserir um novo
                    v_existe := 'N';
                  --
                ...
                IF v_existe = 'S' THEN
                  ...
                  END;
                ELSE -- v_existe = 'N'
                  -- Como não existem datas contiguas devo inserir os novas atividades
                  BEGIN
                    OPEN C_FUN_ATI;
                    --
                    LOOP
              ...
              --
            ELSIF C_FUN.TIP_OPERACAO = 'D' THEN -- Operação é 'D' (Delete)
              BEGIN
                --
              ...
              --
            ELSIF C_FUN.TIP_OPERACAO = 'U' THEN  -- Operação é 'U' (Update)
              BEGIN
                --
                -- Verifico se a função foi alterada
                ...
                IF C_FUN.OLD_DT_INICIO != C_FUN.NEW_DT_INICIO THEN
                  ...
                  --
                END IF;
                -- Faço as atualizações respeito da data de termino
                ...
                THEN
                  BEGIN
                    ...
                    -- que tenha a mesma função e esteja aberta
                    OPEN CUR_EXISTE;
