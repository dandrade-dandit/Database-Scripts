SET SCAN OFF
CREATE OR REPLACE PACKAGE PCK_CONSULTA_COM_FUNCAO AS 
   TYPE TCOM_FUNCAO IS RECORD (
   ...
   TYPE TDADOS IS REF CURSOR RETURN TCOM_FUNCAO;

   PROCEDURE PRC_COM_FUNCAO(TAB IN OUT TDADOS, IDEP_DEP_CODIGO IN NUMBER := NULL, 
                                            IDEP_CODIGO IN NUMBER := NULL, 
                                            IUOR_CODIGO IN NUMBER := NULL,
					    IMATRICULA  IN NUMBER := NULL);
END;
/
CREATE OR REPLACE PACKAGE BODY PCK_CONSULTA_COM_FUNCAO AS 
		  ...
		  S.EMP_QLP_CAR_CODIGO, S.EMP_QLP_CAR_OCC_CODIGO, S.FUN_NOME                  
           ...
           WHERE S.EMP_STATUS <> 2 AND 
		     ...
		     (SELECT C.FUN_CODIGO FROM CARGOS_CONFIANCA C
		      WHERE C.FUN_POSICAO_TRABALHO <> 1 AND
