...
CREATE OR REPLACE 
PROCEDURE prc_gerar_solic_pagamento (
...
--
         BEGIN
            SELECT ifrbde_views.seq_num_seq_contrato.NEXTVAL
...
--
/*                         raise_application_error (-20100,
                         ' v_val_retirada '||to_char(v_val_retirada)
                           ||' v_valor_base_pag '||to_char(v_valor_base_pag)
                           ||' v_retencao '||to_char(v_val_retencao)
                           ||' p_retencao '||to_char(p_val_retencao_rp)
                           ||' percentual ' ||to_char(NVL (
                                                       p_val_retencao_rp,
                                                       0
                                                    )
                                                  / NVL (v_valor_base_pag, 0)));
*/
                        IF v_qtd_recursos > 1
                        THEN
...
GRANT EXECUTE ON prc_gerar_solic_pagamento TO bde_geral
/
