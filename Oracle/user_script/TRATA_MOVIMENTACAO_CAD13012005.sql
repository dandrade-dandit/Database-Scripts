...
PROCEDURE TRATA_MOVIMENTACAO_CAD(p_dep              in number,
  ...
  Begin
     ...
     exception
        when others then
          par_cod_erro := 1;
          par_msg_erro := 'Erro ao inserir rubrica '||to_char(v_rubrica)||' na movimentacao. '||to_char(sqlcode)||' - '||sqlerrm;
     end;
  End;
  /* AGREGADO 07/03/2003 */
  --------------------------------------------------------------------------
  ...
  --
     ...
     if to_number(to_char(p_dt_ini_ant,'yyyymm')) <
        ...
        -- Agregado em 21/08/2003
        -- Se p_dt_fim_ant no se corresponde com o mes da folha aberto então os dias
        -- trabalhados no mes da folha aberta são zero.
        if to_char(p_dt_fim_ant,'yyyymm') != to_char(v_data_ref,'yyyymm') then
          v_dias_trabalhados := 0;
        else -- to_char(p_dt_fim_ant,'yyyymm') = to_char(v_data_ref,'yyyymm')
        -- Fim Agregado em 21/08/2003
  ...
  Procedure trata_desligamento_mes_ant is
    ...
    Begin
      -- pega codigo de rubrica segum tipo remuneracao e tipo de ocorrencia
      if p_ofu_codigo IN (89,220) then -- é substituição de função
        v_rub_codigo := 1020;
      elsif p_ofu_codigo IN (85,217) then -- é acumulação de função
        v_rub_codigo := 1030;
      else -- é função
        if p_tp_remunera = 'FG' then
          v_rub_codigo := 1015;
      ...
      delete
       ...
       where MOV_ANO_REFERENCIA = v_ano_ref
         ...
         and MOV_EMP_NUMERO_MATRICULA = p_matricula;
      --
      /* Agregado em 15/10/2003 */
      if v_rub_codigo = 1010 then
        update MOVIMENTACAO
  ...
  Begin
     ...
     if p_dt_fim_ocor is null or
        ...
        if to_number(to_char(p_dt_ini_ocor,'yyyymm')) =
           ...
           if v_dias_trabalhados = 0 then  -- e se a data de início for dia 31 então daria valor igual a zero
              v_dias_trabalhados := 1; -- vamos transformar o valor para 1 (um) dia
           end if;
  ...
  -- Verifica se folha já foi paga.
     ...
     loop
        ...
        if v_pagto_mes = 'SIM' and user != 'SRH_SFP' then
           exit;
        end if;
     end loop;
     close c_pago;
