...
PROCEDURE ACERTA_PA_FERIAS( p_mat            NUMBER,
   ...
   CURSOR c_afastamento IS
      ...
      AND    (oem_data_termino is null or  oem_data_termino >= oem_data_inicio)-- to_char(oem_data_termino,'yyyy') > 2001 )
...
--       DBMS_OUTPUT.PUT_LINE('Afastamento data_f='||to_char(data_f,'dd/mm/yyyy hh24:mi:ss'));
         IF TO_NUMBER(TO_CHAR(periodo_f, 'YYYY')) > (TO_NUMBER(TO_CHAR(SYSDATE, 'YYYY')) + 2) THEN
            --
            periodo_i := NULL;
            periodo_f := NULL;
            EXIT;
