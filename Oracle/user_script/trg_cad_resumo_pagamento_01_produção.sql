/* Formatted on 2007/04/26 14:16 (Formatter Plus v4.8.0) */
...
--
      BEGIN
         SELECT a.flg_moeda_controle, b.tip_situacao_contrato,
                ...
                b.cnt_data_inicio,
                (  b.dat_termino_contrato
                 + (c.num_prazo_execucao_contrato - c.num_prazo_execucao)
                )
           ...
           FROM ifrbde.cad_resumo_valores a,
                ...
                ifrbde.cad_pre_compromisso c
          WHERE a.num_seq_resumo = :NEW.num_seq_pag_contrato_rp
            AND a.num_seq_contrato_res = b.num_seq_contrato
            AND a.num_seq_contrato_res = c.seq_contrato;

--
         v_liberacao_sp := 'F';
--
         ifrbde.prc_liberacao_realizacao_sp (:NEW.num_seq_resumo_pag,
                                             :NEW.cod_tipo_pag_rp,
                                             v_carencia,
                                             v_num_contrato,
...
--
      EXCEPTION
         ...
         THEN
            v_erro :=
                 'Não foi possível localizar o resumo do contrato' || SQLERRM;
            v_sucesso := 1;
      END;
...
--
      ...
      THEN
--
         ifrbde.prc_ano_realizacao_sp ('I',
...
--
         IF :NEW.cod_tipo_pag_rp IN ('P', 'R', 'D')
         THEN
            ifrbde.prc_gerar_solic_pagamento (v_num_pgose,
                                              :NEW.num_seq_resumo_pag,
...
--
            v_erro :=
               ...
               || TO_CHAR (:NEW.dat_vencimento_pag, 'dd/mm/yyyy')
               || ' , igual a valor de realização = '
               || TO_CHAR ((:NEW.val_pagamento_rp - :NEW.val_glosa_rp
--                            - :NEW.val_retencao_rp
                           ))
               || ' , e tipo '
...
--
               SELECT num_seq_resumo_crono
                ...
                WHERE dat_vencimento_rc = :NEW.dat_vencimento_pag
                  AND val_resumo_crno =
                                   (:NEW.val_pagamento_rp - :NEW.val_glosa_rp
--                          - :NEW.val_retencao_rp
                                   )
...
--
                           INSERT INTO cad_resumo_cronograma
                                ...
                                VALUES (v_num_seq_resumo_crono_novo,
                                        :NEW.num_seq_pag_contrato_rp,
                                        :NEW.dat_vencimento_pag,
                                        NVL ((  :NEW.val_pagamento_rp
                                              - :NEW.val_glosa_rp
--                                              - :new_val_retencao_rp
                                             ),
                                             0
                                            ),
                                        DECODE (:NEW.cod_tipo_pag_rp,
                                                'P', 'DE',
                                                'R', 'RJ',
                                                'D', 'DA'
                                               ),
...
--                          - :NEW.val_retencao_rp
                  ...
                  OR (v_dat_vencimento_crono <> :OLD.dat_vencimento_pag)
               THEN
                  v_erro :=
                     'Os dados de pagamento (valor da fatura - valor Glosa - valor Retenção), ou data de vencimento ou tipo pagamento da SP não podem ser modificados , exclua a SP e atualize o cronograma de realização';
                  v_sucesso := 1;
               ELSE
                  IF :NEW.dat_vencimento_pag <> :OLD.dat_vencimento_pag
                  THEN
--
                     UPDATE cad_resumo_cronograma
                        SET num_seq_resumo_pag = NULL
                      WHERE num_seq_resumo_crono = v_num_seq_resumo_crono_r;

--
                     UPDATE cad_resumo_cronograma
                        SET dat_vencimento_rc = :NEW.dat_vencimento_pag
                      WHERE num_seq_resumo_crono = v_num_seq_resumo_crono_r;

--
                     UPDATE cad_resumo_cronograma
                        SET num_seq_resumo_pag = :OLD.num_seq_resumo_pag
                      WHERE num_seq_resumo_crono = v_num_seq_resumo_crono_r;
--
                  END IF;
               END IF;
...
--
      IF v_sucesso = 0
      THEN
         ifrbde.prc_ano_realizacao_sp ('E',
...
--
         ...
         EXCEPTION
            WHEN OTHERS
            THEN
               v_erro := 'Erro ao excluir lançamento ' || SQLERRM;
               v_sucesso := 1;
         END;
...
--
         ...
         THEN
            BEGIN
               ifrbde.prc_exclui_sst_contrato_sp (v_num_contrato,
                                                  v_num_versao_contrato,
                                                  :OLD.num_fatura_rp
                                                 );
            EXCEPTION
...
--
      v_liberacao_sp := 'F';
--
      ifrbde.prc_liberacao_realizacao_sp (:OLD.num_seq_resumo_pag,
                                          :OLD.cod_tipo_pag_rp,
                                          v_carencia,
                                          v_num_contrato,
                                          :OLD.dat_vencimento_pag,
