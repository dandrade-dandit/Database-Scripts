create or replace procedure CALCULA_PREVIA_FOLHA(par_mes_calculo number,
...
----------------------------------------------------------------------
  ...
  cursor c_infraprev is
    ...
    where  ars_plano = v_empregado.egt_plano_arsaprev
...
Begin
   ...
   /*for i in arinfraprev.first..arinfraprev.last  loop
     if p_plano = arinfraprev(i).ars_plano then
        if arinfraprev(i).ars_faixa = arinfraprev(arinfraprev.last).ars_faixa then
...
Procedure obtem_valor_valadicFer is
   CURSOR c_a13f (p_matricula in number) IS
     ...
     from  (select a13f_valor valor
			      ...
			      union all
			      SELECT round(NVL(SUM(NVL(a13f.a13f_valor,0)),0),2)/12/30*r_mov.mov_parametro valor
			      FROM   ifrsrh_folha.adicional_13_ferias a13f
			      WHERE  a13f.a13f_emp_numero_matricula = p_matricula
