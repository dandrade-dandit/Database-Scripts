...
# |              the traditional export utility. By default, this script       |
# |              performs a full and consistent backup of the database using a |
# |              dynamically created parameter file that gets written to a     |
# |              temporary directory and run through Oracle export using the   |
# |              "parfile" parameter.                                          |
# |                                                                            |
# |              --------------------------------------------                  |
...
# |                                                                            |
# |              ----------------------------                                  |
# |              LABEL SECURITY AND VPD USERS                                  |
# |              ----------------------------                                  |
# |              When exporting data from an Oracle database that contains     |
# |              tables protected by Fine-Grained Access Control (FGAC)        |
# |              policies, it is possible to receive EXP-00079 and/or          |
# |              EXP-00080 warnings.                                           |
# |                                                                            |
# |              Note that Fine-Grained Access Control (FGAC) is a synonym for |
# |              Row-Level Security (RLS) and should not be confused with      |
# |              FGA which stands for Fine-Grained Auditing!                   |
# |                                                                            |
# |              This warning is thrown by Oracle export when FGAC is enabled  |
# |              on a SELECT statement and indicates that Oracle export may    |
# |              not export the entire table because FGAC access may rewrite   |
# |              the query. There are two methods used to resolve this issue   |
# |              and ensure Oracle export is able to access and backup all     |
# |              data:                                                         |
# |                                                                            |
# |                  (1) Use the Direct Path clause of Oracle Export or        |
# |                  (2) Use a database login that has access to all rows      |
# |                      regardless of existing FGAC policies.                 |
# |                                                                            |
# |              It is highly recommended that the latter option be used       |
# |              especially when exporting tables that contain objects and     |
# |              LOBs. Rows in tables that contain objects and LOBs will be    |
# |              exported using the Conventional Path method, even if Direct   |
# |              Path was specified. If this table has FGAC policies enabled,  |
# |              the export will then fail with EXP-00008, ORA-00604, and      |
# |              ORA-28112 errors. The recommended method is to run Oracle     |
# |              export while connected as a user who has access to all rows   |
# |              regardless of existing FGAC policies. Only the user           |
# |              SYS (all versions) or any user who has the EXEMPT ACCESS      |
# |              POLICY privilege (Oracle9i and higher), can select all rows.  |
# |              The recommended convention is to export using the database    |
# |              user BACKUP_ADMIN which has been granted the                  |
# |              EXEMPT ACCESS POLICY privilege:                               |
# |                                                                            |
# |              SQL> GRANT exempt access policy TO backup_admin;              |
# |                                                                            |
# |              Note that this section does not apply to the                  |
# |              Oracle Database 10g Data Pump Export utility (expdp). An      |
...
# |              TARGET_DBA_USERNAME  Database username EXP will use to login  |
# |                                   to the database. This user must be a     |
# |                                   DBA, or must have the EXP_FULL_DATABASE  |
# |                                   or IMP_FULL_DATABASE roles. A common     |
# |                                   username for performing Oracle backups   |
# |                                   is BACKUP_ADMIN.                         |
# |              TARGET_DBA_PASSWORD  Database password used to log in to the  |
# |                                   target database.                         |
# |              EXP_DUMP_DIR         File system directory path used by       |
# |                                   Oracle export to write the dump file(s)  |
# |                                   to on the database server. Note that     |
...
# |                                   naming the Oracle dump file using the    |
# |                                   following naming convention:             |
# |                                                                            |
# |                                   {SCRIPT_NAME}_{UNIQUE_ID}_{HOSTNAME}_{DATE}.dmp
# |                                                                            |
# |              NUM_DAYS_TO_KEEP     Number of days worth of Oracle export    |
...
function printScriptParameterVariables {
    ...
    wl "TRACE> EXP_DUMP_DIR         set to $EXP_DUMP_DIR"
...
wl "EXP_DUMP_DIR                    (P5) : $EXP_DUMP_DIR"
wl "NUM_DAYS_TO_KEEP                (P6) : $NUM_DAYS_TO_KEEP"
...
wl "| ${DATE_PRINT_LOG}                                                     |"
wl "|-------------------------------------------------------------------------|"
wl "| SET FURTHER ENVIRONMENT VARIABLES FOR ORACLE EXPORT.                    |"
wl "+-------------------------------------------------------------------------+"

EXP_DATE_LOG=`date +%Y%m%d_%H%M`
EXP_DUMP_FILE_NAME=${EXP_DUMP_DIR}/${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_${EXP_DATE_LOG}.dmp
EXP_DUMP_LOG_FILE_NAME=${CUSTOM_ORACLE_LOG_DIR}/${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_${EXP_DATE_LOG}_EXPLOG.log
EXP_DUMP_PARFILE_FILE_NAME=${CUSTOM_ORACLE_TEMP_DIR}/${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_${EXP_DATE_LOG}.parfile

wl " "
wl "==========================================================="
wl "      INTERNAL ORACLE EXPORT ENVIRONMENT VARIABLES         "
wl "==========================================================="
wl "EXP_DUMP_DIR                    : $EXP_DUMP_DIR"
wl "EXP_DUMP_FILE_NAME              : $EXP_DUMP_FILE_NAME"
wl "EXP_DUMP_LOG_FILE_NAME          : $EXP_DUMP_LOG_FILE_NAME"
wl "EXP_DUMP_PARFILE_FILE_NAME      : $EXP_DUMP_PARFILE_FILE_NAME"
wl "==========================================================="
...
${RM_BIN} -f $EXP_DUMP_LOG_FILE_NAME | ${TEE_BIN} -a $LOG_FILE_NAME
${RM_BIN} -f $EXP_DUMP_PARFILE_FILE_NAME | ${TEE_BIN} -a $LOG_FILE_NAME
...
buffer=50000000
file=${EXP_DUMP_FILE_NAME}
log=${EXP_DUMP_LOG_FILE_NAME}
compress=n
grants=y
indexes=y
direct=no
...
triggers=y
statistics=none
constraints=y" > $EXP_DUMP_PARFILE_FILE_NAME
...
$ORACLE_HOME/bin/exp parfile=$EXP_DUMP_PARFILE_FILE_NAME 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME
...
wl "| IGNORE 'EXP-00079: Data in table \"<TABLE_NAME>\" is protected.'          |"
wl "| APPLY RETENTION POLICY FOR OBSOLETE EXPORT (DUMP) FILES.                |"
wl "+-------------------------------------------------------------------------+"

${EGREP_BIN} 'EXP-|ORA-|JMA-' $LOG_FILE_NAME | ${EGREP_BIN} -v 'EXP-00079: Data in table'
...
then
    ...
    wl "TRACE> | both the Export dump files and log files.     |"
    wl "TRACE> +-----------------------------------------------+"

    wl " "
    wl "+-------------------------------------------------------------------------+"
    wl "| REMOVING OBSOLETE EXPORT DMP FILES (greater than $NUM_DAYS_TO_KEEP days old)"
    wl "|   (${EXP_DUMP_DIR}/${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_*.dmp)"
    wl "+-------------------------------------------------------------------------+"
    ${FIND_BIN} ${EXP_DUMP_DIR}/ -name "${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_*.dmp" -daystart -mtime +${NUM_DAYS_TO_KEEP} -exec ${LS_BIN} -l {} \; | ${TEE_BIN} -a $LOG_FILE_NAME
    ${FIND_BIN} ${EXP_DUMP_DIR}/ -name "${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_*.dmp" -daystart -mtime +${NUM_DAYS_TO_KEEP} -exec ${RM_BIN} -rf {} \; | ${TEE_BIN} -a $LOG_FILE_NAME

    wl " "
    wl "+-------------------------------------------------------------------------+"
    wl "| REMOVING OBSOLETE EXPORT LOG FILES (greater than $LOG_FILE_ARCHIVE_OBSOLETE_DAYS days old)"
    wl "|   (${CUSTOM_ORACLE_LOG_DIR}/${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_*_EXPLOG.log)"
    wl "+-------------------------------------------------------------------------+"
    ${FIND_BIN} ${CUSTOM_ORACLE_LOG_DIR}/ -name "${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_*_EXPLOG.log" -mtime +${LOG_FILE_ARCHIVE_OBSOLETE_DAYS} -exec ${LS_BIN} -l {} \; | ${TEE_BIN} -a $LOG_FILE_NAME
    ${FIND_BIN} ${CUSTOM_ORACLE_LOG_DIR}/ -name "${SCRIPT_NAME_NOEXT}_${UNIQUE_SCRIPT_IDENTIFIER}_${HOSTNAME_SHORT_UPPER}_*_EXPLOG.log" -mtime +${LOG_FILE_ARCHIVE_OBSOLETE_DAYS} -exec ${RM_BIN} -rf {} \; | ${TEE_BIN} -a $LOG_FILE_NAME
