...
# |----------------------------------------------------------------------------|
# | DATABASE   : Oracle                                                        |
# | FILE       : archive_oracle_log_files.ksh                                  |
# | CLASS      : UNIX Shell Scripts                                            |
# | PURPOSE    : This script is responsible for archiving and compressing a    |
# |              select list of Oracle log and trace files on this host for a  |
# |              given instance. For example, the alert.log and the global     |
# |              listener.log.                                                 |
# |                                                                            |
# | PARAMETERS :                                                               |
# |              TARGET_DB_NAME       Database name from v$database.           |
# |              TARGET_SID           Database SID found in the oratab file    |
# |                                   for the target database.                 |
# |              LISTENER_NAME        [optional] - TNS listener name found in  |
# |                                   the listener.ora file. Example TNS       |
# |                                   listener names:                          |
# |                                       LISTENER                             |
# |                                       LISTENER_<machine_name>              |
# |                                                                            |
# | EXAMPLE    :                                                               |
# |              # RAC Example                                                 |
# |              archive_oracle_log_files.ksh racdb racdb1 listener_racnode1   |
# |                                                                            |
# |              # Stand Alone Example                                         |
# |              archive_oracle_log_files.ksh testdb testdb listener           |
# |                                                                            |
# |              # Stand Alone / Do No Archive the TNS Listener Log            |
# |              archive_oracle_log_files.ksh testdb testdb                    |
# |                                                                            |
...
if [[ -z $LISTENER_NAME ]]; then
    EXPECTED_NUM_SCRIPT_PARAMS=2
    ARCHIVE_LISTENER_LOG="NO"
else
    EXPECTED_NUM_SCRIPT_PARAMS=3
    ARCHIVE_LISTENER_LOG="YES"
fi

UNIQUE_SCRIPT_IDENTIFIER=${TARGET_SID}
...
function printScriptParameterVariables {
    ...
    wl "TRACE> LISTENER_NAME    set to $LISTENER_NAME"
...
function showUsage {
    ...
    $L_SHOW " "
    $L_SHOW "    optional"
    $L_SHOW "    parameters:  listener_name"
    $L_SHOW " "
...
wl "LISTENER_NAME        (P3)            : $LISTENER_NAME"
wl " "

wl "==========================================================="
wl "                  CUSTOM SCRIPT VARIABLES                  "
wl "==========================================================="
wl "ARCHIVE_LISTENER_LOG                 : $ARCHIVE_LISTENER_LOG"
...
wl "| ${DATE_PRINT_LOG}                                                     |"
wl "|-------------------------------------------------------------------------|"
wl "| ARCHIVE ALERT.LOG FILE.                                                 |"
wl "+-------------------------------------------------------------------------+"

(cd $ORACLE_ADMIN_DIR/$TARGET_DB_NAME/bdump; ${CP_BIN} -v alert_$TARGET_SID.log "alert_$TARGET_SID.$START_DATE_LOG.log") 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME

(cd $ORACLE_ADMIN_DIR/$TARGET_DB_NAME/bdump; ${ZIP_BIN} alert_$TARGET_SID.$START_DATE_LOG.log.zip "alert_$TARGET_SID.$START_DATE_LOG.log") 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME

${RM_BIN} -v "$ORACLE_ADMIN_DIR/$TARGET_DB_NAME/bdump/alert_$TARGET_SID.$START_DATE_LOG.log" 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME

${CAT_BIN} /dev/null > $ORACLE_ADMIN_DIR/$TARGET_DB_NAME/bdump/alert_$TARGET_SID.log 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME
...
wl "| ${DATE_PRINT_LOG}                                                     |"
wl "|-------------------------------------------------------------------------|"
wl "| ARCHIVE LISTENER.LOG FILE.                                              |"
wl "+-------------------------------------------------------------------------+"

if [[ $ARCHIVE_LISTENER_LOG = "NO" ]]; then

    wl " "
    wl "The Oracle TNS listener.log file was not archived as per user request."

else

    $ORACLE_HOME/bin/lsnrctl <<EOF | ${TEE_BIN} -a $LOG_FILE_NAME
set current_listener $LISTENER_NAME
set log_status off
exit
EOF

    ${MV_BIN} -v "$ORACLE_HOME/network/log/$LISTENER_NAME.log" "$ORACLE_HOME/network/log/$LISTENER_NAME.$START_DATE_LOG.log" 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME

    (cd $ORACLE_HOME/network/log; ${ZIP_BIN} $LISTENER_NAME.$START_DATE_LOG.log.zip "$LISTENER_NAME.$START_DATE_LOG.log") 2>&1 | ${TEE_BIN} -a $LOG_FILE_NAME

    ${RM_BIN} -v "$ORACLE_HOME/network/log/$LISTENER_NAME.$START_DATE_LOG.log"
...
${EGREP_BIN} 'zip error|cannot touch|cannot stat|ORA-|NL-|TNS-' $LOG_FILE_NAME
