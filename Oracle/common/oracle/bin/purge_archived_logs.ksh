...
# |----------------------------------------------------------------------------|
# | DATABASE   : Oracle                                                        |
# | FILE       : purge_archived_logs.ksh                                       |
# | CLASS      : UNIX Shell Scripts                                            |
# | PURPOSE    : Used to delete obsolete archived redo log files from a        |
# |              database using RMAN. This script was designed and tested with |
# |              Oracle10g. Although this script may work with versions higher |
# |              than Oracle10g, it may not take advantage of newly introduced |
# |              features.                                                     |
# |                                                                            |
# |              This script assumes the target instance exists on the server  |
# |              running this script.                                          |
# |                                                                            |
# |              -----------------------------                                 |
# |              RMAN BINARY                                                   |
...
# |              Recovery Area" in Oracle10g, most database configurations do  |
# |              not set any RMAN configuration parameters.                    |
# |                                                                            |
# | PARAMETERS :                                                               |
...
# |                                     the target database.                   |
# |              NUM_DAYS_TO_KEEP       Number of days worth of archived redo  |
# |                                     logs to be retained.                   |
# |                                                                            |
# |                                                                            |
# | EXAMPLE RUN:                                                               |
# |              purge_archived_logs.ksh thingdb1 thingdb1 backup_admin backup_admin_pwd 5 > purge_archived_logs_thingdb1_backup_admin_THING1.job
# |                                                                            |
...
wl "NUM_DAYS_TO_KEEP                (P5) : $NUM_DAYS_TO_KEEP"
...
wl "| ${DATE_PRINT_LOG}                                                     |"
wl "|-------------------------------------------------------------------------|"
wl "| PURGE ARCHIVE REDO LOGS (sysdate - ${NUM_DAYS_TO_KEEP})                 "
wl "+-------------------------------------------------------------------------+"

$ORACLE_HOME/bin/rman target "${TARGET_DBA_USERNAME}/${TARGET_DBA_PASSWORD}@${TARGET_DB_NAME}" nocatalog <<EOF | tee -a $LOG_FILE_NAME
    list archivelog all;
    delete noprompt force archivelog until time 'sysdate - ${NUM_DAYS_TO_KEEP}';
    exit;
EOF
...
${EGREP_BIN} 'ORA-|ERROR MESSAGE STACK FOLLOWS' $LOG_FILE_NAME
