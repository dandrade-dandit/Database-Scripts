/* Formatted on 2005/02/16 10:13 (Formatter Plus v4.5.2) */

CREATE OR REPLACE PROCEDURE ifrbde.prc_calcula_hst_pag_crono_vers (
...
--
-- AJUSTA OS VALORES DOS SALDO NO CRONOGRAMA NA MESMA VERSAO DO CONTRATO
--
   FOR regx IN rega
   LOOP
      v_sucesso := 0;
...
--
         ...
         THEN
            v_erro :=
                  ...
                  || ' Fatura = '
                  || regx.num_fatura_pagamento
                  || ' Valor da Glosa = '
                  || TO_CHAR (regx.val_glosa)
                  || ' cod resumo =  '
                  || TO_CHAR (regx.num_seq_resumo_contrato)
                  || ' tipo do cronograma =  '
                  || '"'
...
EXCEPTION
   ...
   THEN
      ...
      v_sucesso := 1;
      raise_application_error (-20100, v_erro);
END prc_calcula_hst_pag_crono_vers; -- Procedure
/



-- End of DDL Script for Procedure IFRBDE.PRC_CALCULA_HST_PAG_CRONO_VERS
