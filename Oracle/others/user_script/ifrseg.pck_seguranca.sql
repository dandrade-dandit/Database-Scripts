/* Formatted on 21/02/2005 09:24 (Formatter Plus v4.8.0) */
...
---------------------------------------------------------------------------------------------------------------------
   ...
   PROCEDURE stp_validar (
      ...
      CURSOR c_usuario (
         v_sessionid                IN       NUMBER,
         v_sistema                  IN       VARCHAR2
      ) IS
         SELECT   acs_usuario
             FROM ifrseg.viw_tab_acesso
            WHERE acs_sessionid = v_sessionid
              AND acs_sistema = v_sistema
         ORDER BY acs_data DESC;
   BEGIN
      vo_usuario (1) := '';
      v_sessionid :=
              TO_NUMBER (pck_seguranca.stf_separausername (p_sessionid, 'S'));
...
-- Armazena Log de acesso
   ...
   BEGIN
      INSERT INTO ifrseg.viw_tab_log
           ...
           VALUES (TO_NUMBER (pck_seguranca.stf_separausername (p_sessionid,
                                                                'S'
                                                               )
                             ),
                   SYSDATE,
...
/*      v_sessionid :=
               TRIM (SUBSTR (p_usuario, v_ind1 + 1, (v_ind2 - (v_ind1 + 1))));
*/
      v_sistema :=
