-- Start of DDL Script for Trigger IFRBDE.TRG_CAD_RESUMO_VALORES_01
-- Generated 2-dez-2004 20:23:55 from SYSMAN@HOMOLOG2

CREATE OR REPLACE TRIGGER ifrbde.trg_cad_resumo_valores_01
...
--
      ...
      :NEW.cod_recurso_res := NVL (:NEW.cod_recurso_res, '00000000000000');
      ...
      IF :NEW.val_resumo = 0
      THEN
         v_erro := ' Valor do resumo não pode ser 0(zero).';
         v_sucesso := 1;
      END IF;


--
      ifrbde.pck_variaveis.v_pck_num_seq_contrato := :NEW.num_seq_contrato_res;
...
--
      ...
      THEN
         v_erro :=
               ...
               || '-> Saldo  : '
               || TO_CHAR (NVL (:NEW.val_saldo_resumo_sec, 0))
               || '= Resumo  : '
               || TO_CHAR (NVL (:NEW.val_resumo_sec, 0))
               || '+ Termo   : '
               || TO_CHAR (NVL (:NEW.val_termo_sec, 0))
               || '- faturado: '
               || TO_CHAR (NVL (:NEW.val_pago_sec, 0));
         v_sucesso := 1;
      END IF;
...
--
      ...
      THEN
         v_erro :=    'O Saldo do Resumo de valores não pode ser negativo '
                   || '-> Saldo  : '
                   || TO_CHAR (NVL (:NEW.val_saldo_resumo, 0))
                   || '= Resumo  : '
                   || TO_CHAR (NVL (:NEW.val_resumo, 0))
                   || '+ Termo   : '
                   || TO_CHAR (NVL (:NEW.val_termo, 0))
                   || '- faturado: '
                   || TO_CHAR (NVL (:NEW.val_pago, 0));
         v_sucesso := 1;
      END IF;
...
-- End of DDL Script for Trigger IFRBDE.TRG_CAD_RESUMO_VALORES_01
