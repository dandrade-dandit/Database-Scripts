/* Formatted on 2006/03/13 13:54 (Formatter Plus v4.8.0) */
CREATE OR REPLACE PROCEDURE ifrbde.prc_conf_identificacao_u (
...
--
         IF regs.programa = 'S'
         THEN
            IF     SUBSTR (TRIM (UPPER (regs.tipo_dependencia)), 1, 2) = 'SR'
            ...
            ELSE
               ...
               ELSE
                  ...
                  THEN
                     v_custo := '201309';
                  ELSE
                     IF     SUBSTR (TRIM (UPPER (regs.tipo_dependencia)),
                                    1,
                                    3
                                   ) = 'GNA'
                        AND TRIM (UPPER (regs.sigla_dependencia)) <> 'SEDE'
...
--
            raise_application_error
                       (-20100,
                           ' Erro ao verificar qual o custo para o contrato '
                        || ' -  dependencia :'
                        || TRIM (UPPER (regs.sigla_dependencia))
                        || ' -  Tipo de Dependencia : '
                        || SUBSTR (TRIM (UPPER (regs.tipo_dependencia)), 1, 3)
                        || ' -  Programa : '
                        || regs.programa
                       );
--
         ...
         BEGIN
            ...
            UPDATE integracao.conf_identificacao_u
               SET payable_entity_id = v_entidade,
                   ...
                   contract_sub_type = NVL (regs.cod_subtipo_contrato, '0001'),
                   contract_id_vendor =
                      v_projeto || ' Carencia => '
                      || TO_CHAR (regs.cnt_cre_cod),
                   codigo_mod_contratacao = regs.cod_modalidade_licitacao,
                   inciso = regs.num_inciso,
                   fundamento_legal = regs.dsc_fundamento_legal,
                   manager_id = v_custo,
                   security_code = ' ',
                   sistema_origem_code = '02',
                   contract_versao_origem = v_num_versao_anterior,
                   regularidade_fiscal = regs.tip_regularidade_fiscal,
            ...
            THEN
               INSERT INTO integracao.conf_identificacao_u
                           (payable_entity_id, contract_id,
                            contract_versao,
                            contract_desc,
                            contract_status_code, contract_prev_status_code,
                            contract_type,
                            contract_sub_type,
                            contract_id_vendor,
                            codigo_mod_contratacao, inciso,
                            fundamento_legal, manager_id, security_code,
                            sistema_origem_code, contract_versao_origem,
                            regularidade_fiscal, date_created,
                    ...
                    VALUES (v_entidade, v_contrato_id,
                            ...
                            SUBSTR (regs.cnt_justificativa_cre, 1, 70),
                            'IC', 'IC',
                            TRIM (SUBSTR (v_contrato_id, 8, 2)),
                            NVL (regs.cod_subtipo_contrato, '0001'),
                               v_projeto
                            || ' Carencia => '
                            || TO_CHAR (regs.cnt_cre_cod),
                            regs.cod_modalidade_licitacao, regs.num_inciso,
                            regs.dsc_fundamento_legal, v_custo, ' ',
                            '02', v_num_versao_anterior,
                            regs.tip_regularidade_fiscal, SYSDATE,
                            USER, SYSDATE, USER,
                            '0', SYSDATE, 0
                           );
            WHEN OTHERS
...
--GRANT EXECUTE ON ifrbde.prc_conf_identificacao_u TO bde_geral
--/
-- End of DDL Script for Procedure IFRBDE.PRC_CONF_IDENTIFICACAO_U
