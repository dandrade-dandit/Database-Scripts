...
--------------------
  PROCEDURE PRC_COPIA_VERSAO
             (
              ...
              p_NUM_ANO_D                         number,
              p_COD_NIVEL_D                       varchar2,
              p_COD_DEPENDENCIA_AGRUPADORA_D      number,
              p_COD_VERSAO_D                      varchar2,
...
-------------------------
    ...
    cursor q_VSP
      ...
      select vsp.CD_VERSAO  CD_VERSAO
        ...
        where vsp.NR_ANO = p_NUM_ANO
              ...
              vsp.IN_ESCOLHIDA = 1;
    --
    r_VSP_ESCOLHIDA  q_VSP%rowtype;
    l_VSP_ESCOLHIDA  boolean;
    --
  ...
  BEGIN
    ...
    ----------------------------------------------------------
    -- PASSO 05 - DEPENDENCIA + VINCULADAS PARA VERIFICAÇÃO --
    ----------------------------------------------------------
    for f_DEP_VERIFICACAO in q_DEP
                              (
                               p_NUM_ANO,
    ...
    LOOP
      -------------------------------------------------
      -- PASSO 02 - PESQUISA DE VERSÃO NÃO ESCOLHIDA --
      -------------------------------------------------
      open q_VSP
            (
             p_NUM_ANO,
             p_COD_NIVEL-1,
             f_DEP_VERIFICACAO.DEP_CODIGO
            );
      fetch q_VSP into r_VSP_ESCOLHIDA;
      l_VSP_ESCOLHIDA := q_VSP%found;
      close q_VSP;
      -----------------------------------------------------
      -- PASSO 03 - DEPENDÊNCIA COM VERSÃO NÃO ESCOLHIDA --
      -----------------------------------------------------
      IF not l_VSP_ESCOLHIDA
      THEN
        --------------------------------------
        -- PASSO 04 - INTERRUPÇÃO POR DADOS --
        --------------------------------------
        p_IND_RETORNO := false;
        p_DSC_RETORNO := 'A dependência '||f_DEP_VERIFICACAO.DEP_CODIGO||' não foi escolhida.';
        GOTO FIM;
      END IF;
    END LOOP;
    --------------------------------------------------------------
    ...
    EXCEPTION
      ...
      THEN
        p_IND_RETORNO := false;
        p_DSC_RETORNO := 'A dependência '||f_DEP_VERIFICACAO.DEP_CODIGO||' não foi escolhida.'||CHR(10)||
                         sqlerrm;
        GOTO FIM;
    END;
    ------------------------------------------------------
    ...
    LOOP
      ...
      THEN
        ...
        --------------------------------------
        p_IND_RETORNO := false;
        p_DSC_RETORNO := 'Na Dependência '||f_dep_geracao.DEP_CODIGO||
                         ', nível '||p_COD_NIVEL-1||
                         ', versão '||r_vsp_geracao.CD_VERSAO||
                         ' não informado dados de despesa.';
        GOTO FIM;
      END IF;
      --------------------------------------------
