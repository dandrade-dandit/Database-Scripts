-- Start of DDL Script for Package Body IFRBDE.PCK_CONTRATO
-- Generated 2-dez-2004 20:35:46 from SYSMAN@HOMOLOG2

CREATE OR REPLACE 
PACKAGE BODY ifrbde.pck_contrato
IS
...
--
            ifrbde.prc_sincroniza_carencia (v_cad_resumo_pagamento.num_seq_pag_contrato_rp,'E');
--
...
-- retorna o recordset correspondente
   ...
   AS
      ...
      v_tab_firma   ifrbde.tab_firma%ROWTYPE;
      v_dado        VARCHAR2 (4000)            := NULL;
   BEGIN
      vo_ok (1) := 0;

      IF ifrseg.pck_seguranca.stf_permitido (p_usuario, 'TAB_FIRMA', 'EXCLUI')
      THEN
         OPEN c_cursor;
         ...
         THEN
            v_dado :=    'fir_cod = '
                      || TO_CHAR (v_tab_firma.fir_cod);
            v_dado :=    v_dado
                      || ','
                      || 'tip_cnpj_cpf = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.tip_cnpj_cpf)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_cnpj = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_cnpj)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_nome = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_nome)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_razao = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_razao)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_contato = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_contato)));
            v_dado :=    v_dado
                      || ','
                      || 'dsc_endereco = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.dsc_endereco)));
            v_dado :=    v_dado
                      || ','
                      || 'dsc_complemento = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.dsc_complemento)));
            v_dado :=    v_dado
                      || ','
                      || 'dsc_bairro = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.dsc_bairro)));
            v_dado :=    v_dado
                      || ','
                      || 'dsc_cidade = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.dsc_cidade)));
            v_dado :=    v_dado
                      || ','
                      || 'num_cep = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.num_cep)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_fone = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_fone)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_fax = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_fax)));
            v_dado :=    v_dado
                      || ','
                      || 'sg_uf = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.sg_uf)));
            v_dado :=    v_dado
                      || ','
                      || 'sg_pais = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.sg_pais)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_email = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_email)));
            v_dado :=    v_dado
                      || ','
                      || 'fir_obs = '
                      || RTRIM (LTRIM (UPPER (v_tab_firma.fir_obs)));
...
-- retorna o recordset correspondente
   ...
   BEGIN
      IF ifrseg.pck_seguranca.stf_permitido (p_usuario, 'TAB_FIRMA', 'INCLUI')
      THEN
         ...
         IF  v_numero_limite = 9999 AND v_sucesso = 0
         THEN
         ...
         THEN
            ...
            BEGIN
               INSERT INTO ifrbde.tab_firma
                           (fir_cod, tip_cnpj_cpf, fir_cnpj,
                            fir_nome, fir_obs)
                    VALUES (v_fir_cod, v_tip_cnpj_cpf, v_fir_cnpj,
                            v_fir_nome, v_fir_obs);
            EXCEPTION
               WHEN OTHERS
               THEN
                  raise_application_error (
...
-- retorna o recordset correspondente
   ...
   AS
      ...
      v_tab_firma      ifrbde.tab_firma%ROWTYPE;
      v_fir_cod        ifrbde.tab_firma.fir_cod%TYPE;
      v_tip_cnpj_cpf   ifrbde.tab_firma.tip_cnpj_cpf%TYPE;
      v_fir_cnpj       ifrbde.tab_firma.fir_cnpj%TYPE;
      v_fir_nome       ifrbde.tab_firma.fir_nome%TYPE;
      v_fir_obs        ifrbde.tab_firma.fir_obs%TYPE;
      v_dado           VARCHAR2 (4000)                      := NULL;
   BEGIN
      IF ifrseg.pck_seguranca.stf_permitido (p_usuario, 'TAB_FIRMA', 'ALTERA')
      THEN
         OPEN c_cursor;
         FETCH c_cursor INTO v_tab_firma;
      ...
      THEN
         ...
         INSERT INTO ifrbde.viw_contrato
                     (num_seq_contrato, cnt_cre_cod, cnt_cod,
                      ...
                      dsc_fundamento_legal, num_licitacao,
                      num_ano_licitacao, num_processo,
                      tip_regularidade_fiscal, tip_situacao_contrato)
              VALUES (v_num_seq_contrato, v_cnt_cre_cod, v_cnt_cod,
                      ...
                      v_tip_regularidade_fiscal, v_tip_situacao_contrato);
