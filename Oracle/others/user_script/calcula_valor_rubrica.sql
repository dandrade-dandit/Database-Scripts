create or replace procedure Calcula_Valor_Rubrica(par_mes_calculo number, par_ano_calculo number, par_versao number,
   		                        par_matricula number, par_rub_codigo number, par_mov_sequencial number,
                                par_mov_for_codigo number, par_mov_parcela number, par_mov_parcela_atual number,
                                par_mov_tipo_parametro varchar2, par_mov_parametro number, par_mov_valor in out number,
                                par_mov_tip_rubrica varchar2, par_mov_mes_ref_diferenca number,
                                par_mov_ano_ref_diferenca number, par_mov_valor_integral in out number,
                                par_mov_dias_proporcionalidade number, par_mov_fun_codigo number,
                                par_mov_tipo_funcao varchar2, par_mov_nsa_codigo_nivel number,
                                par_mov_nsa_codigo_padrao varchar2, par_mov_oem_emp_numero_matricu number,
                                par_mov_oem_ofu_codigo number, par_mov_oem_data_inicio date,
                                par_mov_nsa_indicador_nivel_es varchar2, par_valor_fgts in out number) is
   ----------------------------------------------------------------------
   -- Procedure que calcula a prévia dos valores das rubricas na       --
   -- movimentação do empregado, no mês/ano-versão                     --
   --                                                                  --
   -- Criada 	        em 27/11/2003                                  --
   -- AUTORES : Luiz Fernando Bastos Coura                             --
   ----------------------------------------------------------------------
     v_empregado              empregados_pagamentos%rowtype;
     cursor c_mov_mat is
       select par_ano_calculo mov_ano_referencia, par_mes_calculo mov_mes_referencia, par_versao mov_versao_pagamento,
              par_matricula mov_emp_numero_matricula, par_rub_codigo mov_rub_codigo, par_mov_sequencial mov_sequencial,
              par_mov_for_codigo mov_for_codigo, par_mov_parcela mov_parcela, par_mov_parcela_atual mov_parcela_atual,
              par_mov_tipo_parametro mov_tipo_parametro, par_mov_parametro mov_parametro, par_mov_valor mov_valor, par_mov_tip_rubrica mov_tip_rubrica,
              par_mov_mes_ref_diferenca mov_mes_ref_diferenca, par_mov_ano_ref_diferenca mov_ano_ref_diferenca,
              'N' mov_ind_calculo, par_mov_valor_integral mov_valor_integral,
              par_mov_dias_proporcionalidade mov_dias_proporcionalidade, par_mov_fun_codigo mov_fun_codigo, par_mov_tipo_funcao mov_tipo_funcao,
              par_mov_nsa_codigo_nivel mov_nsa_codigo_nivel, par_mov_nsa_codigo_padrao mov_nsa_codigo_padrao,
              par_mov_oem_emp_numero_matricu mov_oem_emp_numero_matric, par_mov_oem_ofu_codigo mov_oem_ofu_codigo, par_mov_oem_data_inicio mov_oem_data_inicio,
              par_mov_nsa_indicador_nivel_es mov_nsa_indicador_nivel_esc
       from   dual;
     cursor c_faixa is
     ...
     cursor c_salfam is
       ...
       from   salario_familia, (select max(to_date(to_char(saf_mes_referencia)||'/'||
                                ...
                                where  to_number(to_char(saf_ano_referencia)||lpad(to_char(saf_mes_referencia),2,'0')) <= to_number(to_char(par_ano_calculo)||lpad(to_char(par_mes_calculo),2,'0'))) table_data
       where  to_date(to_char(saf_mes_referencia)||'/'||to_char(saf_ano_referencia),'MM/YYYY') = table_data.data
     ...
     cursor c_infraprev is
       ...
       from   faixa_arsaprev, (select max(to_date(to_char(ars_mes)||'/'||
                               ...
                               from   faixa_arsaprev
                               where  to_number(to_char(ars_ano)||lpad(to_char(ars_mes),2,'0')) <= to_number(to_char(par_ano_calculo)||lpad(to_char(par_mes_calculo),2,'0'))
     ...
     cursor c_irrf is
       ...
       from   impostos_rendas, (select max(to_date(to_char(imr_mes_referencia)||'/'||to_char(imr_ano_referencia),'MM/YYYY')) data
                                from   impostos_rendas
                                where  to_number(to_char(imr_ano_referencia)||lpad(to_char(imr_mes_referencia),2,'0')) <=
                                       to_number(to_char(par_ano_calculo)||lpad(to_char(par_mes_calculo),2,'0'))) table_data
       where  to_date(to_char(imr_mes_referencia)||'/'||to_char(imr_ano_referencia),'MM/YYYY') = table_data.data
       order  by imr_renda_maxima;
     r_mov                     c_mov_mat%rowtype;
   ...
   --
     cursor c_constant is
            ...
            from   constantes_legais,
                  (select max(to_date(to_char(col_mes_referencia)||'/'||to_char(col_ano_referencia),'MM/YYYY')) data
                   from   constantes_legais
                   where  to_number(lpad(to_char(col_ano_referencia),4,'0')||lpad(to_char(col_mes_referencia),2,'0')) <=
                          to_number(lpad(to_char(par_ano_calculo),4,'0')||lpad(to_char(par_mes_calculo),2,'0'))) table_data
            where  to_date(to_char(col_mes_referencia)||'/'||to_char(col_ano_referencia),'MM/YYYY') = table_data.data;
   ...
   Begin
     ...
     and    mov.mov_rub_codigo in
           (select iru_rub_codigo_incide
            ...
            and   (data_termino is null or
       	        to_number(to_char(data_termino,'yyyymmdd')) >= to_number(lpad(to_char(par_ano_calculo),4,'0')||
                             lpad(to_char(par_mes_calculo),2,'0')||lpad(to_char(01),2,'0'))));
     -- Se o valor do somarubinc retornou zero é porque empregado se encontrava afastado busca valor integral para empregados afastados
     if NVL(v_valor_operando,0) = 0 then
        select nvl(sum(to_number(decode(rub_tipo_rubrica||mov.mov_tip_rubrica,
                       'RN','+','RD','+','RE','-','DN','-','DD','-','DE','+')||
                       to_char(nvl(mov.mov_valor_integral,0)))),0)
        into   v_valor_operando
        from   movimentacao mov, rubricas
        where  mov.mov_ano_referencia       = r_mov.mov_ano_referencia
        and    mov.mov_mes_referencia       = r_mov.mov_mes_referencia
     ...
     if r_mov.mov_rub_codigo = 5067 then  -- rubrica de INFRAPREV PLANO CDFGTS
   ...
   Begin
       ...
       from   faixas_inss, (select max(to_date(to_char(fai_mes_referencia)||'/'||
                            ...
                            where  to_number(to_char(fai_ano_referencia)||
                                        lpad(to_char(fai_mes_referencia),2,'0')) <=
                                             to_number(to_char(par_ano_calculo)||lpad(to_char(par_mes_calculo),2,'0'))) table_data
       where  to_date(to_char(fai_mes_referencia)||'/'||to_char(fai_ano_referencia),'MM/YYYY') = table_data.data;
   Exception
     when others then
        grava_log_erro;
   End;
   ...
   Begin
     ...
     and    mov.mov_rub_codigo in
           (select iru_rub_codigo_incide
            ...
            and   (data_termino is null or
       	        to_number(to_char(data_termino,'yyyymmdd')) >=
                   to_number(lpad(to_char(par_ano_calculo),4,'0')||lpad(to_char(par_mes_calculo),2,'0')||lpad(to_char(01),2,'0'))));
     if v_valor_operando < 0 then
        v_valor_operando := 0;
     end if;
   Exception
   ...
   Begin
      ...
      from   faixa_arsaprev, (select max(to_date(to_char(ars_mes)||'/'||
                              ...
                              where  to_number(to_char(ars_ano)||lpad(to_char(ars_mes),2,'0')) <=
                                     to_number(to_char(par_ano_calculo)||lpad(to_char(par_mes_calculo),2,'0'))
       ...
       and    mov.mov_rub_codigo in
             (select iru_rub_codigo_incide
              ...
              and  ((mov.mov_tip_rubrica = 'N' and iru_tip_normal      = 'S') or
                    (mov.mov_tip_rubrica = 'D' and iru_tip_diferenca   = 'S') or
                    (mov.mov_tip_rubrica = 'E' and iru_tip_estorno_dev = 'S'))
              and   (data_termino is null or to_number(to_char(data_termino,'yyyymmdd')) >=
                     to_number(lpad(to_char(par_ano_calculo),4,'0')||lpad(to_char(par_mes_calculo),2,'0')||lpad(to_char(01),2,'0'))));
     Exception
       when others then
         grava_log_erro;
          ...
          and    cfv.cfv_data_vigencia in
                (select max(cfv1.cfv_data_vigencia)
                 ...
                 and    to_number(to_char(cfv1.cfv_data_vigencia,'yyyymm')) <=
                        to_number(lpad(to_char(par_ano_calculo),4,'0')||lpad(to_char(par_mes_calculo),2,'0')));
        Exception
          when no_data_found then
            v_valor_operando := 0;
          when others then
   ...
   Begin
     ...
     Begin
        ...
        and    hns.hns_nsa_indicador_nivel_escola = r_mov.mov_nsa_indicador_nivel_esc
        and    hns.hns_data_vigencia =
              (select max(hns.hns_data_vigencia)
               from   historico_niveis_salariais hns
               where  hns_nsa_indicador_nivel_escola = r_mov.mov_nsa_indicador_nivel_esc
     ...
     if r_mov.mov_tipo_funcao = 'RG' then
        ...
        v_valor_operando := (v_valor_salario_tit + v_funcao_titular) - (v_valor_salario_sub + v_valor_func_sub);
     end if;
     if v_valor_operando < 0 then
        v_valor_operando := 0;
     end if;
   ...
   Begin
     ...
     v_num_dias := to_number(to_char(last_day(to_date(to_char(v_mes_anterior,'00')||'/'||to_char(v_ano_anterior,'0000'),'MM/YYYY')),'DD'));
     Begin
   ...
   --    Verifica existência de função, periculosidade, insalubridade e recupera acumulado em empregados_ferias_13 se não existir no mês.
      v_rub_codigo := 1010;
      obtem_valor_valrub;
      ...
      if v_valor_1010 = 0 then
         begin
           ...
           and   (to_number(to_char(efe_ano_referencia)||lpad(to_char(efe_mes_referencia),2,'0')) >= v_ano_mes_ini and
                     to_number(to_char(efe_ano_referencia)||lpad(to_char(efe_mes_referencia),2,'0')) <= v_ano_mes_fim)
      ...
      v_valor_operando := (v_valor_13_ferias * v_valor_dia) + v_valor_1010 + v_valor_1015 + v_valor_1085 + v_valor_1090;
   Exception
     when others then
        grava_log_erro;
   End;
   ...
   --   recupera ocorrência de férias (074) com o mês e ano de cálculo utilizando o campo oem_mov_parametro.
      begin
         ...
         and    oem_mov_parametro = to_number(lpad(to_char(par_ano_calculo),4,'0')||lpad(to_char(par_mes_calculo),2,'0'));
      exception
         when no_data_found then
            insert into evento_folha
   ...
   Begin
     if r_mov.mov_for_codigo is not null then
        ...
        Begin
          ...
          and    to_date(to_char(fru.fru_mes_inicio)||'/'||to_char(fru.fru_ano_inicio),'MM/YYYY') in
                (select max(to_date(to_char(fru1.fru_mes_inicio)||'/'||to_char(fru1.fru_ano_inicio),'MM/YYYY'))
                 from   formulas_rubricas fru1
                 where  fru1.fru_rub_codigo = r_mov.mov_rub_codigo
                 and    to_number(lpad(to_char(fru1.fru_ano_inicio),4,'0')||lpad(to_char(fru1.fru_mes_inicio),2,'0')) <=
                        to_number(lpad(to_char(par_ano_calculo),4,'0')||lpad(to_char(par_mes_calculo),2,'0')))
     ...
     while instr(v_formula, ' ') > 0 loop
       if substr(v_formula, 1, 1) in ('*', '/', '+', '-', '(', ')') then
          ...
          if v_operando = 'SALARIOBAS' then
             ...
             v_rub_codigo := TO_NUMBER(SUBSTR(v_operando, 11,4));
          ...
          v_formula_com_valores := v_formula_com_valores ||to_char(v_valor_operando) || ' ';
       end if;
     end loop;
   Exception
     when others then
   ...
   Begin
     if r_mov.mov_emp_numero_matricula <> nvl(v_matricula_anterior,0) then
        Begin
           select egt_dependentes_ir, egt_dependentes_sal_fam,
                  ...
                  egt_plano_arsaprev
           into   v_empregado.egt_dependentes_ir, v_empregado.egt_dependentes_sal_fam,
                  v_empregado.egt_numero_mes_13, v_empregado.egt_numero_anuenios,
                  v_empregado.egt_carga_horaria_mes, v_empregado.egt_base_fgts,
                  v_empregado.egt_base_inss, v_empregado.egt_base_irrf,
                  v_empregado.egt_percentual_arsaprev, v_empregado.egt_plano_arsaprev
     ...
     if nvl(r_mov.mov_tipo_parametro,'D') <> 'V' then
        ...
        if v_formula_com_valores <> ' ' then
           begin
              v_valor_rubrica := sql_dinamico('select trunc('||replace(v_formula_com_valores,',','.')||',2)from dual');
           exception
             when others then
                grava_log_erro;
           end;
        end if;
        if r_mov.mov_mes_referencia = 12 and r_mov.mov_rub_codigo in (1390,1340,1345,5090,5098,5085,5095,5060,5510,5525,5105) then
           null;
        end if;
     else
     ...
     if r_mov.mov_rub_codigo in (5100,5105,5110,5115) then
        Obtem_valor_irrf;
     end if;
     ...
     if nvl(r_mov.mov_dias_proporcionalidade,0) < 30 or
        ...
        v_fator_proporcional := nvl(r_mov.mov_dias_proporcionalidade,0);
        begin
           par_mov_valor := (v_valor_rubrica/30) * v_fator_proporcional;
           par_mov_valor_integral := v_valor_rubrica;
        exception
          when others then
            grava_log_erro;
        end;
