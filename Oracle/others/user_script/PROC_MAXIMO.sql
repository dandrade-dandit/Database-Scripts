...
BEGIN
   ...
   FOR REG_SISMAT IN CUR_RM LOOP
	 ...
	 IF REG_SISMAT.IN_ABERTA = '0' THEN
            V_STATUS1 := 'Aberta p/ solicitação e atendimento';
       	 END IF;

	 IF REG_SISMAT.IN_ABERTA= '1' THEN
	    V_STATUS1 := 'Fechada p/ solicitação e Aberta p/ atendimento';
       	 END IF;

         IF REG_SISMAT.IN_ABERTA = '2' THEN
            V_STATUS1 := 'Aberta p/ item inédito e Fechada para atendimento';
         END IF;
				      ...
				      '62','ALMOXSBRJ','20','ALMOXSBMO'); 
					   ...
					   COMMIT;
			             ...
			             COMMIT;
                           ...
                           END;
                        END IF;                                
                     END; -- SE NÃO EXISTIR NA INVENTORY

               END; --  SE NÃO EXISTIR NA ITEM
		    ...
		    SELECT DECODE(C.CD_ESTOCAVEL, '1', 'EST', '0', 'NE') ESTOCAVEL,
		       ...
		       AND  b.DEP_CD_DEPENDENCIA     = REG_SISMAT.DEP_CD_DEPENDENCIA));
                    INSERT INTO IFRMAXIMO.INVENTORY(
                       ...
                       '0');
                        COMMIT;
               END; -- SE EXISTIR NA ITEM E NÃO EXISTIR NA INVENTORY
            ...
            BEGIN
	         ...
	         IF (REG_SISMAT.qt_atendida <> '0')  THEN
		       ...
		       COMMIT;
               	    ...
               	    END;
                 END IF; -- condição de atendimento <> 0      
            END;
         END IF; -- condição de status <> de Fechada p/ atendimento       
      END; -- BEGIN DO FOR
   END LOOP;  -- FOR
END; -- FINAL DO BLOCO
