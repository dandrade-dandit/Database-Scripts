create or replace package body PCK_ANALISE
IS
  ------------------------------------------------------
  ...
  IS
    v_CENTRO_CUSTO_SOLICITANTE  number(5);
    ...
    cursor q_RRE
      ...
      where rre.SEQ_ANO_ORCAMENTO = p_SEQ_ANO_ORCAMENTO
            ...
            and
            substr(rre.NUM_CONTA_CONTABIL,1,length(p_NUM_CONTA_SOLICITANTE)) = p_NUM_CONTA_SOLICITANTE
     order by rre.NUM_CONTA_CONTABIL;
    r_RRE  q_RRE%rowtype;
    l_RRE  boolean;
    -------------------------
    -- CONSULTA - PASSO 05 --
    ...
    -----------------------------------------------------------
    -- CONSULTA - PASSO 16 - VERIFICAÇÃO - DEPENDÊNCIA É GNA --
    -----------------------------------------------------------
    cursor q_DEP
            (
             p_COD_DEPENDENCIA  number
            )
      is
      select 'GNA'
        from DEPENDENCIAS  dep
        where dep.DEP_CODIGO = p_COD_DEPENDENCIA
              and
              dep.DEP_TIPO = 6;
  r_DEP  q_DEP%rowtype;
  l_DEP  boolean;
  ...
  BEGIN
    ...
    -----------------------------------------------------------
    -- PASSO 02 - ITEM SOLICITANTE(S) + PROGRAMAÇÃO - ORIGEM --
    -----------------------------------------------------------
    ...
    LOOP
      ------------------------------------------------------------------------
      -- PASSO 03 - LOCALIZAÇÃO - ORÇAMENTO REALIZADO (DEPENDÊNCIA E CONTA) --
      ------------------------------------------------------------------------
      open q_RRE
            (
             p_SEQ_ANO_ORCAMENTO,
      ...
      close q_RRE;
      ----------------------------------------------------------------------------------
      -- PASSO 04 - CONDIÇÃO - ORÇAMENTO REALIZADO (DEPENDÊNCIA E CONTA) - ENCONTRADO --
      ----------------------------------------------------------------------------------
      IF    l_RRE = true
      THEN
        -----------------------------------------------------
        -- PASSO 05 - IR PARA - RATEIO POR CENTRO DE CUSTO --
        -----------------------------------------------------
        GOTO RATEIO_POR_CENTRO_CUSTO;
      ---------------------------------------------------------------------------------------
      -- PASSO 06 - CONDIÇÃO - ORÇAMENTO REALIZADO (DEPENDÊNCIA E CONTA) - NÃO ENCONTRADO  --
      ---------------------------------------------------------------------------------------
      ELSIF l_RRE = false
      THEN
        ---------------------------------------------------------------------
        -- PASSO 07 - CONDIÇÃO - CONTA GESTORA É ALMOXARIFADO ( 11301005 ) --
        ---------------------------------------------------------------------
        IF p_CC1_NR_CONTA = '11301005'
        THEN
          ---------------------------------------------------------------------
          -- PASSO 08 - LOCALIZAÇÃO - ORÇAMENTO REALIZADO 1a.CONTA DE 31104% --
          ---------------------------------------------------------------------
          open q_RRE
                (
                 p_SEQ_ANO_ORCAMENTO,
          ...
          close q_RRE;
          -------------------------------------------------------------------------------
          -- PASSO 09 - CONDIÇÃO - ORÇAMENTO REALIZADO 1a.CONTA DE 31104% - ENCONTRADO --
          -------------------------------------------------------------------------------
          IF l_RRE
          THEN
            -----------------------------------------------------
            -- PASSO 10 - IR PARA - RATEIO POR CENTRO DE CUSTO --
            -----------------------------------------------------
            GOTO RATEIO_POR_CENTRO_CUSTO;
          -----------------------------------------------------------------------------------
          -- PASSO 11 - CONDIÇÃO - ORÇAMENTO REALIZADO 1a.CONTA DE 31104% - NÃO ENCONTRADO --
          -----------------------------------------------------------------------------------
          ELSIF not l_RRE
          THEN
            -----------------------------------------------------------------------------
            -- PASSO 12 - INSERÇÃO SOLICITANTE - CUSTO 20110 E CONTA 31104001 ( 100% ) --
            -----------------------------------------------------------------------------
            open q_DEP
                  (
                   f_isp_origem.COD_DEPENDENCIA_SOLICITANTE
            ...
            close q_DEP;
            ---------------------------------------------
            -- PASSO 13 - CONDIÇÃO - DEPENDÊNCIA - GNA --
            ---------------------------------------------
            IF    l_DEP = TRUE
            THEN
              ---------------------------------------------------
              -- PASSO 14 - DEFINIÇÃO DE CENTRO DE CUSTO - GNA --
              ---------------------------------------------------
              v_CENTRO_CUSTO_SOLICITANTE := 14110;
            -------------------------------------------------
            -- PASSO 15 - CONDIÇÃO - DEPENDÊNCIA - NÃO GNA --
            -------------------------------------------------
            ELSIF l_DEP = FALSE
            THEN
              -------------------------------------------------------
              -- PASSO 16 - DEFINIÇÃO DE CENTRO DE CUSTO - NÃO GNA --
              -------------------------------------------------------
              v_CENTRO_CUSTO_SOLICITANTE := 20110;
            END IF;
            -------------------------------------------------
            -- PASSO 17 - INCLUSÃO DO SOLICITANTE ( 100% ) --
            -------------------------------------------------
            BEGIN
              insert
                ...
                values
                 (
                  ...
                  v_CENTRO_CUSTO_SOLICITANTE,
                  f_isp_origem.VAL_TOTAL_SOLICITANTE
                 );
            EXCEPTION
              WHEN OTHERS
              THEN
                p_IND_RETORNO := false;
                p_DSC_RETORNO := 'Pas: 17,'||
                                 'Ano: '||p_NUM_ANO||','||
                                 'Nív: '||p_COD_NIVEL||','||
                                 'DAg: '||p_COD_DEPENDENCIA_AGRUPADORA||','||
                                 'Ver: '||p_COD_VERSAO_DESTINO||','||
                                 'DEl: '||p_COD_DEPENDENCIA_ELABORACAO||','||
                                 'Ite: '||p_NR_ITEM||','||
                                 'Err: '||sqlerrm;
              GOTO FIM;
            END;
            -----------------------------------------------------
            -- PASSO 18 - ATUALIZAÇÃO DE CÁLCULO - SOLICITANTE --
            -----------------------------------------------------
            p_VAL_TOTAL_SOLICITANTE := p_VAL_TOTAL_SOLICITANTE + f_isp_origem.VAL_TOTAL_SOLICITANTE;
            -----------------------------------------------
            -- PASSO 19 - INCLUSÃO DA PROGRAMAÇÃO (100%) --
            -----------------------------------------------
            BEGIN
              insert
                ...
                values
                 (
                  ...
                  p_NUM_ANO,
                  f_isp_origem.VAL_JANEIRO,
                  f_isp_origem.VAL_FEVEREIRO,
                  f_isp_origem.VAL_MARCO,
                  f_isp_origem.VAL_ABRIL,
                  f_isp_origem.VAL_MAIO,
                  f_isp_origem.VAL_JUNHO,
                  f_isp_origem.VAL_JULHO,
                  f_isp_origem.VAL_AGOSTO,
                  f_isp_origem.VAL_SETEMBRO,
                  f_isp_origem.VAL_OUTUBRO,
                  f_isp_origem.VAL_NOVEMBRO,
                  f_isp_origem.VAL_DEZEMBRO,
                  0,
                  0,
                  0,
                  0,
            ...
            END;
            -----------------------------------------------------
            -- PASSO 20 - ATUALIZAÇÃO DE CÁLCULO - PROGRAMAÇÃO --
            -----------------------------------------------------
            p_VAL_TOTAL_PROGRAMACAO := p_VAL_TOTAL_PROGRAMACAO + f_isp_origem.VAL_JANEIRO
                                                               + f_isp_origem.VAL_FEVEREIRO
                                                               + f_isp_origem.VAL_MARCO
                                                               + f_isp_origem.VAL_ABRIL
                                                               + f_isp_origem.VAL_MAIO
                                                               ...
                                                               + f_isp_origem.VAL_DEZEMBRO;
            --------------------------------------------
            -- PASSO 21 - IR PARA PROXIMO SOLICITANTE --
            --------------------------------------------
            GOTO FIM_SOLICITANTE;
          ------------------------------------------------------------------------------------------
          -- FIM - CONDIÇÕES - ENCONTRADO/NÃO ENCONTRADO - ORÇAMENTO REALIZADO 1a.CONTA DE 31104% --
          ------------------------------------------------------------------------------------------
          END IF;
        ---------------------------------------------------------------------------------
        -- PASSO 22 - CONDIÇÃO - CONTA GESTORA - NÃO SENDO - ALMOXARIFADO ( 11301005 ) --
        ---------------------------------------------------------------------------------
        ELSE
          --------------------------------------------------------------
          -- PASSO 23 - LANÇAMENTO - SOLICITANTE - ( 100 % = GESTOR ) --
          --------------------------------------------------------------
          BEGIN
            insert
              into ITEM_SOLICITANTE_PROPOSTA
          ...
          END;
          -----------------------------------------------------
          -- PASSO 24 - ATUALIZAÇÃO DE CÁLCULO - SOLICITANTE --
          -----------------------------------------------------
          p_VAL_TOTAL_SOLICITANTE := p_VAL_TOTAL_SOLICITANTE + f_isp_origem.VAL_TOTAL_SOLICITANTE;
          -----------------------------------------------
          -- PASSO 25 - INSERÇÃO DA PROGRAMAÇÃO (100%) --
          -----------------------------------------------
          BEGIN
            insert
              into PROGRAMACAO_SOLIC_PROPOSTA
          ...
          END;
          -----------------------------------------------------
          -- PASSO 26 - ATUALIZAÇÃO DE CÁLCULO - PROGRAMAÇÃO --
          -----------------------------------------------------
          p_VAL_TOTAL_PROGRAMACAO := p_VAL_TOTAL_PROGRAMACAO + f_isp_origem.VAL_JANEIRO
                                                             + f_isp_origem.VAL_FEVEREIRO
                                                             + f_isp_origem.VAL_MARCO
                                                             ...
                                                             + f_isp_origem.VAL_DEZEMBRO;
          --------------------------------------------
          -- PASSO 27 - IR PARA PROXIMO SOLICITANTE --
          --------------------------------------------
          GOTO FIM_SOLICITANTE;
        ------------------------------------------------------------
        -- FIM - CONDIÇÕES - CONTA GESTORA ALMOXARIFADO - SIM/NÃO --
        ------------------------------------------------------------
        END IF;
      -----------------------------------------------------------------------------------------------
      -- FIM - CONDIÇÕES - ORÇAMENTO REALIZADO (DEPENDÊNCIA E CONTA) - ENCONTRADO / NÃO ENCONTRADO --
      -----------------------------------------------------------------------------------------------
      END IF;
      <<RATEIO_POR_CENTRO_CUSTO>>
      ------------------------------
      -- PASSO 26 - MES DE 1 A 12 --
      ------------------------------
      for f_MES in 1..12
      LOOP
        --------------------------------------------------
        -- PASSO 28 - DEFINIÇÃO - VALOR DO MES CORRENTE --
        --------------------------------------------------
        ...
        v_VAL_LANCADO_MES := 0;
        ------------------------------------------------------------------
        -- PASSO 29 - LOOP - RATEIO REALIZADO DETALHE (CENTRO DE CUSTO) --
        ------------------------------------------------------------------
        for f_RRD in q_RRD
                      (
        ...
        LOOP
          ...
          ----------------------------------------------------------------
          -- PASSO 31 - LOCALIZAÇÃO - ITEM SOLICITANTE(S) - DESTINO - I --
          ----------------------------------------------------------------
          open q_ISP_DESTINO
                (
                 p_NUM_ANO,
          ...
          close q_ISP_DESTINO;
          ------------------------------------------------------
          -- PASSO 32 - ITEM SOLICITANTE - DESTINO --
          ------------------------------------------------------
          IF not l_ISP_DESTINO
          THEN
            -------------------------------------------------------------------------------
            -- PASSO 28 - NÄO EXISTE - ITEM SOLICITANTE DESTINO - INSERT (DEFINIR VALOR) --
            -------------------------------------------------------------------------------
            BEGIN
              -------------------------------------------------------------------------------
              -- PASSO 28.1 - NÄO EXISTE - ITEM SOLICITANTE DESTINO - INSERT (DEFINIR VALOR) --
              -------------------------------------------------------------------------------
              insert
                into ITEM_SOLICITANTE_PROPOSTA
                 (
              ...
              -------------------------------------------------------------------------------
              -- PASSO 28.2 - NÄO EXISTE - ITEM SOLICITANTE DESTINO - INSERT (DEFINIR VALOR) --
              -------------------------------------------------------------------------------
  ...
  BEGIN
    ...
    LOOP
      ...
      THEN
        IF f_ips.FLG_LANCAMENTO in ('A','C')
        THEN
          v_FLG_LANCAMENTO := 'R';
	END IF;
      END IF;
      ...
      --====================================--
      -----------------------------------------------
      -- PASSO 05 - CONDIÇÃO - PARA FAZER O RATEIO --
      -----------------------------------------------
      IF p_IND_RATEIO_AUTOMATICO = TRUE
         and
         v_FLG_LANCAMENTO = 'R'
      THEN
        --------------------------------------------
        -- PASSO 06 - CHAMADA - RATEIO AUTOMÁTICO --
        --------------------------------------------
        ...
        ------------------------------------------------------------------------------------
        -- PASSO 07 - CONDIÇÃO - RESULTADO DO PROCESSO - RATEIO AUTOMÁTICO : BEM SUCEDIDO --
        ------------------------------------------------------------------------------------
        IF p_IND_RETORNO = true
        THEN
          --------------------------------------------------------
          -- PASSO 08 - EXISTÊNCIA DE SOLICITANTE E PROGRAMAÇÃO --
          --------------------------------------------------------
          v_IND_EXISTENCIA_SOLICITANTE := true;
          v_IND_EXISTENCIA_PROGRAMACAO := true;
        --------------------------------------------------------------------------------------------------------------------------
        -- PASSO 09 - CONDIÇÃO - RESULTADO DO PROCESSO DO RATEIO AUTOMÁTICO : MAL SUCEDIDO E ORÇAMENTO REALIZADO NÃO ENCONTRADO --
        --------------------------------------------------------------------------------------------------------------------------
        ...
        THEN
          p_IND_RETORNO := TRUE;
          --------------------------------------------
          -- PASSO 10.1 - CONDIÇÃO - CONTA 113.01.005 --
          --------------------------------------------
          IF f_ips.CC1_NR_CONTA <> '11301005'
          THEN
            ----------------------------------------------
            -- PASSO 13 - CALCULO - TOTAL - SOLICITANTE --
            ----------------------------------------------
            v_VAL_TOTAL_SOLICITANTE := f_ips.VAL_TOTAL_GESTOR;
            BEGIN
              -------------------------------------------------------------------
              -- PASSO 14 - INSERÇÃO DE SOLICITANTE - CONFORME CHAVE DO GESTOR --
              -------------------------------------------------------------------
              insert
                into ITEM_SOLICITANTE_PROPOSTA
                 (
            ...
            EXCEPTION
              ...
              THEN
                --------------------------------------------
                -- PASSO 15 - INTERRUPÇÃO - ERRO DE DADOS --
                --------------------------------------------
                open q_ISP_SEQUENCE;
            ...
            END;
            ----------------------------------------------
            -- PASSO 16 - CALCULO - TOTAL - PROGRAMAÇÃO --
            ----------------------------------------------
            ...
            BEGIN
                    --------------------------------------------------
              -- PASSO 17 - INSERÇÃO DE DESPESA / PROGRAMACAO --
              --------------------------------------------------
              insert
                into PROGRAMACAO_SOLIC_PROPOSTA
                 (
      ...
      END IF;
      ----------------------------------------------------------------------
      -- PASSO 19 - AVALIAÇÃO - COPIA NORMAL DE SOLICITANTE E PROGRAMAÇÃO --
      ----------------------------------------------------------------------
      IF p_IND_RATEIO = False
         and
	 v_FLG_LANCAMENTO not in ('A','C')
         or
	 p_IND_RATEIO = True
	 and
         v_FLG_LANCAMENTO not in ('R')
      THEN
        -------------------------------------------------
        -- PASSO 20 - LEITURA DE DESPESA / SOLICITANTE --
        -------------------------------------------------
        for f_ISP in q_ISP
                      (
                       p_NUM_ANO_O,
        ...
        LOOP
          v_IND_EXISTENCIA_SOLICITANTE := true;
          -----------------------------------------------------
          -- PASSO 21 - CALCULO DE TOTALIZAÇÃO / SOLICITANTE --
          -----------------------------------------------------
          v_VAL_TOTAL_SOLICITANTE := v_VAL_TOTAL_SOLICITANTE + f_ISP.VAL_TOTAL_SOLICITANTE;
          ----------------------------------------
          -- PASSO 22 - INSERÇÃO DE SOLICITANTE --
          ----------------------------------------
          BEGIN
            insert
              into ITEM_SOLICITANTE_PROPOSTA
          ...
          EXCEPTION
            ...
            THEN
              --------------------------------------------
              -- PASSO 23 - INTERRUPÇÃO - ERRO DE DADOS --
              --------------------------------------------
              open q_ISP_SEQUENCE;
              fetch q_ISP_SEQUENCE into r_ISP_SEQUENCE;
              close q_ISP_SEQUENCE;
          ...
          END;
          -------------------------------------------------
          -- PASSO 24 - LEITURA DE DESPESA / PROGRAMACAO --
          -------------------------------------------------
          for f_PSP in q_PSP
                        (
          ...
          LOOP
            v_IND_EXISTENCIA_PROGRAMACAO := true;
            -----------------------------------------------------
            -- PASSO 25 - CALCULO DE TOTALIZAÇÃO / PROGRAMACAO --
            -----------------------------------------------------
            ...
            END IF;
            --------------------------------------------------
            -- PASSO 26 - INSERÇÃO DE DESPESA / PROGRAMACAO --
            --------------------------------------------------
            BEGIN
              insert
                into PROGRAMACAO_SOLIC_PROPOSTA
            ...
            EXCEPTION
              ...
              THEN
                --------------------------------------------
                -- PASSO 27 - INTERRUPÇÃO - ERRO DE DADOS --
                --------------------------------------------
                open q_ISP_SEQUENCE;
                fetch q_ISP_SEQUENCE into r_ISP_SEQUENCE;
                close q_ISP_SEQUENCE;
      ...
      END IF;
      ------------------------------------------------------------------------------------
      -- PASSO 28 - AVALIAÇÃO - VERIFICAÇÃO DE DIVERGÊNCIA ENTRE VALOR GESTOR, SOLICITANTE E VALOR  --
      ------------------------------------------------------------------------------------
      ...
      END IF;
      -------------------------------------------------------------
      -- PASSO 32 - VERIFICAÇÃO DE NÃO EXISTÊNCIA DE PROGRAMACAO --
      -------------------------------------------------------------
      IF v_IND_EXISTENCIA_PROGRAMACAO = false
      THEN
        ------------------------------------------------------------------------
        -- PASSO 33 - INTERRUPÇÃO - MENSAGEM DE NÃO EXISTÊNCIA DE PROGRAMACAO --
        ------------------------------------------------------------------------
        p_IND_RETORNO := false;
    ...
    LOOP
      ...
      EXCEPTION
        ...
        THEN
          --------------------------------------------
          -- PASSO 36 - INTERRUPÇÃO - ERRO DE DADOS --
          --------------------------------------------
          p_IND_RETORNO := false;
