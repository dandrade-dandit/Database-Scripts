create or replace procedure        calcula_previa(par_mes_calculo number,
...
----------------------------------------------------------------------
  ...
  cursor c_mov_dep is
    ...
    and    mov.mov_emp_numero_matricula > 0
    and    exists (select 'x'
                   ...
                   and    mov1.mov_versao_pagamento = mov.mov_versao_pagamento
                   and    mov1.mov_emp_dep_codigo_pagto = mov.mov_emp_dep_codigo_pagto
                   and    mov1.mov_emp_numero_matricula = mov.mov_emp_numero_matricula
                   and   (mov1.mov_ind_lancamento_liq_negat = 'N' or mov1.mov_ind_lancamento_liq_negat is null))
  ...
  v_perc_irrf               number := 0;
  v_valor_irrf_092006       number(12,2) := 0;
  v_erro                    varchar2(300);
--
...
Begin
  ...
  if r_mov.mov_ano_referencia = 2006 and r_mov.mov_mes_referencia = 09 and
     r_mov.mov_versao_pagamento = 1  and r_mov.mov_rub_codigo in (5103,5104) then
     v_egt_base_irrf := 0;
     Begin
		    select nvl(sum(to_number(decode(rub_tipo_rubrica||mov.mov_tip_rubrica,
		                   'RN','+','RD','+','RE','-','DN','-','DD','-','DE','+')||
		    ...
		    into   v_valor_operando
		    from   movimentacao  mov, rubricas
		    where  mov.mov_ano_referencia       = decode(r_mov.mov_rub_codigo,5103,r_mov.mov_ano_referencia,5104, r_mov.mov_ano_ref_diferenca)
		    and    mov.mov_mes_referencia       = decode(r_mov.mov_rub_codigo,5103,r_mov.mov_mes_referencia - 1,5104, r_mov.mov_mes_ref_diferenca)
		    and    mov.mov_versao_pagamento     = r_mov.mov_versao_pagamento
		    and    mov.mov_emp_numero_matricula = r_mov.mov_emp_numero_matricula
		    and   (mov.mov_ind_lancamento_liq_negat = 'N' or
		           mov.mov_ind_lancamento_liq_negat is null)
		    ...
		    and    mov.mov_rub_codigo in
		          (select iru_rub_codigo_incide
		           from   incidencias_rubricas
		           where  iru_rub_codigo = decode(r_mov.mov_rub_codigo,5103,5100,5104,5110)
		           and  ((mov.mov_tip_rubrica = 'N' and iru_tip_normal      = 'S') or
		                 (mov.mov_tip_rubrica = 'D' and iru_tip_diferenca   = 'S') or
		                 (mov.mov_tip_rubrica = 'E' and iru_tip_estorno_dev = 'S'))
		           and   (data_termino is null or
     ...
     Begin
       ...
       from   movimentacao  mov, rubricas
       where  mov.mov_ano_referencia       = 2006
       and    mov.mov_mes_referencia       = 9
       and    mov.mov_versao_pagamento     = 3
       and    mov.mov_emp_numero_matricula = r_mov.mov_emp_numero_matricula
       and   (mov.mov_ind_lancamento_liq_negat = 'N' or
              mov.mov_ind_lancamento_liq_negat is null)
       and    mov.mov_rub_codigo           = rub_codigo
...
--  dbms_output.put_line('mov tipo funcao ' || v_tipo_funcao1);
--dbms_output.put_line('rubrica  '|| r_mov.mov_rub_codigo);
...
Begin
  ...
  if v_sfu_codigo = 1 then
     Begin
       ...
       and    hns.hns_nsa_codigo_nivel           = nvl(p_nivel,emp.emp_nsa_codigo_nivel)
       and    hns.hns_nsa_codigo_padrao          = nvl(p_padrao,emp.emp_nsa_codigo_padrao)
       and    hns.hns_nsa_indicador_nivel_escola = nvl(p_escolaridade,emp.emp_nsa_in_nivel_escolaridade)
       and    hns.hns_data_vigencia =
   	         (select max(hns.hns_data_vigencia)
...
--    acumulado em empregados_ferias_13 se não existir no mês.
   ...
   v_valor_operando := (v_valor_13_ferias * v_valor_dia) + v_valor_1010 +
                        v_valor_1015 + v_valor_1085 + v_valor_1090;
Exception
  when others then
     grava_log_erro;
End;
...
--dbms_output.put_line('valor func tit ' || v_valor_func_tit);
...
--  dbms_output.put_line('fun codigo ' || v_fun_codigo);
--  dbms_output.put_line('funcao sub ' || v_valor_func_sub);
...
--dbms_output.put_line(v_valor_operando);
Exception
  when others then
     grava_log_erro;
End;
...
Procedure obtem_valor_valadicFer is
   CURSOR c_a13f (p_ano_inicial in number,
                  p_mes_inicial in number,
                  p_ano_final   in number,
                  p_mes_final   in number,
                  p_dias_ferias in number,
                  p_matricula   in number) IS
      select sum(a.valor)
      from (select a13f_valor valor
            from   adicional_13_ferias
            where  a13f_ano = p_ano_final
            and    a13f_mes = p_mes_final
            and    a13f_sequencial = 99
            and    a13f_emp_numero_matricula = p_matricula
            and    a13f_cod_rubrica = 1350
            union all
            SELECT round(NVL(SUM(NVL(a13f.a13f_valor,0)),0),2)/12/30*p_dias_ferias valor
            FROM   adicional_13_ferias a13f
            WHERE  to_number(lpad(a13f.a13f_ano,4,'0')||lpad(a13f.a13f_mes,2,'0'))
                   between to_number(lpad(p_ano_inicial,4,'0')||lpad(p_mes_inicial,2,'0'))
                   and to_number(lpad(p_ano_final,4,'0')||lpad(p_mes_final,2,'0'))
            and    a13f.a13f_sequencial = 3
            and    a13f.a13f_emp_numero_matricula = p_matricula
            GROUP  BY a13f.a13f_emp_numero_matricula) a;
   ...
   r_valadicFer c_valadicFer%rowtype;

Begin
   open c_valadicFer(r_mov.mov_emp_numero_matricula, r_mov.mov_oem_data_inicio);
   fetch c_valadicFer into r_valadicFer;
   close c_valadicFer;
   if r_valadicFer.termino_pa = last_day(r_valadicFer.termino_pa) and months_between(r_valadicFer.termino_pa + 1, r_valadicFer.inicio_pa) >= 12 then
      r_valadicFer.termino_pa := add_months(r_valadicFer.termino_pa, 1);
   end if;
   atualiza_adicional_ferias (to_number(to_char(r_valadicFer.inicio_pa,'yyyy')), -- par_ano_ini
                              to_number(to_char(r_valadicFer.inicio_pa,'mm')),   -- par_mes_ini
                              to_number(to_char(r_valadicFer.termino_pa,'yyyy')),-- par_ano_fim
                              to_number(to_char(r_valadicFer.termino_pa,'mm')),  -- par_mes_fim
                              r_mov.mov_versao_pagamento,
                              r_mov.mov_emp_numero_matricula);

   consolida_adicional_ferias (r_mov.mov_ano_referencia,
                               r_mov.mov_mes_referencia,
                               r_mov.mov_versao_pagamento,
                               to_number(to_char(r_valadicFer.inicio_pa,'yyyy')), -- par_ano_ini
                               to_number(to_char(r_valadicFer.inicio_pa,'mm')),   -- par_mes_ini
                               to_number(to_char(r_valadicFer.termino_pa,'yyyy')),-- par_ano_fim
                               to_number(to_char(r_valadicFer.termino_pa,'mm')),  -- par_mes_fim
                               LAST_DAY(to_date('01/'||lpad(r_mov.mov_mes_referencia,2,'0')||'/'||
							                           lpad(r_mov.mov_ano_referencia,4,'0'),'dd/mm/yyyy')), -- par_data_parametro
                               r_valadicFer.dias_direito, -- par_dias_ferias
                               r_mov.mov_emp_numero_matricula);
   v_valor_operando := 0;
   OPEN  c_a13f (to_number(to_char(r_valadicFer.inicio_pa,'yyyy')),
                 to_number(to_char(r_valadicFer.inicio_pa,'mm')),
                 to_number(to_char(r_valadicFer.termino_pa,'yyyy')),
                 to_number(to_char(r_valadicFer.termino_pa,'mm')),
                 r_valadicFer.dias_direito,
                 r_mov.mov_emp_numero_matricula);
   FETCH c_a13f into v_valor_operando;
   CLOSE c_a13f;
   --
...
Begin
  -- Recupera base de cálculo da rubrica 5067 ou 5092.
  ...
  -- Recupera base de cálculo da rubrica 1995 para compor juntamente com a rubrica 5067/5092.
  v_rub_codigo := 1995;
  obtem_valor_valrub;
  if v_valor_operando > 0 then
  ...
  -- Recupera valor da rubrica 5067/5092 e aplica o percentual no parâmetro.
  if par_mes_calculo = 13 then
     v_rub_codigo := 5092;
  else
     v_rub_codigo := 5067;
...
Begin
  ...
  if v_valor_rubrica = 0 then
     update movimentacao
     set    mov_tip_rubrica = r_mov.mov_tip_rubrica
...
--     (1390,1340,1345,5090,5098,5085,5095,5060,5510,5525,5105)
  ...
  if r_mov.mov_mes_referencia = 12 then
     ...
     if r_mov.mov_rub_codigo in (5100,5103,5104,5105,5110,5115) and nvl(r_mov.mov_tipo_parametro,'D') <> 'V' then
        Obtem_valor_irrf;
        Obtem_valor_numdepir;
        Obtem_valor_deducaodep;
        if r_mov.mov_ano_referencia = 2006 and r_mov.mov_mes_referencia = 9 and r_mov.mov_versao_pagamento = 1 and r_mov.mov_rub_codigo in (5103,5104) then
           Obtem_valor_baseirrf;
           v_perc_irrf := 0;
           Obtem_valor_percirrf;
           v_perc_irrf := to_number(v_valor_operando);
           Obtem_valor_pardeduzir;
           v_valor_operando := (v_base_irrf  * v_perc_irrf/100) - v_parcela_deduzir;
           v_valor_irrf_092006 := 0;
           begin
             select sum(mov_valor)
             into   v_valor_irrf_092006
             from   movimentacao
             where  mov_ano_referencia       = decode(r_mov.mov_rub_codigo,5103,r_mov.mov_ano_referencia, 5104,r_mov.mov_ano_ref_diferenca)
             and    mov_mes_referencia       = decode(r_mov.mov_rub_codigo,5103,r_mov.mov_mes_referencia - 1, 5104,r_mov.mov_mes_ref_diferenca)
             and    mov_versao_pagamento     = r_mov.mov_versao_pagamento
             and    mov_emp_numero_matricula = r_mov.mov_emp_numero_matricula
             and    mov_rub_codigo           = decode(r_mov.mov_rub_codigo,5103,5100,5104,5110)
             and    mov_tip_rubrica          = 'N';
           exception
             when no_data_found then
               v_valor_irrf_092006 := 0;
           end;
           v_valor_rubrica := nvl(v_valor_operando,0) - nvl(v_valor_irrf_092006,0);
        end if;
...
Begin
   ...
   if par_matricula is null then
      if par_mes_calculo = 12 and par_versao = 1 then
      	 --
   	     -- apaga todos as rubricas de 13º salário para inseri-las
   	     -- novamente igual as rubricas da versão 2 do mês 13
