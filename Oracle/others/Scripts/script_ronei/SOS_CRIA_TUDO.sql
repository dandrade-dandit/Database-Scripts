...
-- parametrização necessaria
 ...
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._01not_found.sql
-- confere existência das tabelas 
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._02refer.sql
-- gera comando para habilitar grant references  
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._03tab.sql
-- gera comando para criação da tabela
 select '--### gera comando para criação da tabela' from dual;

DECLARE
 virg       varchar2(1);
 my_own     dba_tab_columns.owner%type;
 my_tab     dba_tab_columns.table_name%type;
 my_col     dba_tab_columns.column_name%type;
 my_typ     dba_tab_columns.data_type%type;
 my_len     dba_tab_columns.data_length%type;
 my_pre     dba_tab_columns.data_precision%type;
 my_sca     dba_tab_columns.data_scale%type;

 cursor c1 is
   select B.owner,B.table_name
   from dba_tables B, tabelas&dblink A
 ...
 cursor c2 is
          ...
          where owner = my_own and table_name = my_tab
          order by column_id;
BEGIN 
 OPEN C1;
 LOOP
    FETCH C1 inTO my_own, my_tab; 
    EXIT WHEN C1%NOTFOUND;
    dbms_output.put_line ('--'); 
    dbms_output.put_line ('CREATE TABLE '||my_own||'.'||my_tab||' ('); 
    begin
        OPEN C2;
        virg := ' ';
        LOOP
           fetch c2 into my_col,my_typ,my_len,my_pre,my_sca;
           exit when c2%notfound;
           dbms_output.put (virg); 
           if my_typ = 'DATE' or my_typ = 'LONG' or my_typ = 'LONG RAW' then
              dbms_output.put_line (my_col||' '||my_typ); 
           else
              if my_typ = 'VARCHAR2' or my_typ = 'CHAR' then
                 dbms_output.put_line (my_col||' '||my_typ||'('||my_len||')'); 
              else
                 if my_typ = 'NUMBER' then
                    dbms_output.put_line (my_col||' NUMBER('||my_pre||','||my_sca||')'); 
                 else
                    dbms_output.put_line ('##############  DATATYPE NÃO PREVISTO ##################### '||my_typ); 
                 end if;
              end if;
           end if;
           virg := ','; 
...
-- gera comando para criação e inserção dos dados em uma tabela auxiliar 
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._04pk.sql
...
/
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._05fk.sql
...
/
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._06uk.sql
...
/
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._07ck.sql
-- gera comando para criação das constraints check
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._08ind.sql
...
/
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._09rebu_ind.sql
...
/
 ...
 set maxdata 60000;
 set long 32767;
spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._10viw.sql
 -- gera views
 select ' ' from dual;
 select '--### gera comando para criação das views' from dual;
 select 'CREATE OR REPLACE '||V.view_name,
  V.text
  from all_views V, tabelas&dblink A
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._11fun.sql
 -- gera functions
 select ' ' from dual;
 select '--### gera comando para criação das functions' from dual;
 select S.text
  from all_source S, tabelas&dblink A
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._12pkg.sql
 -- gera package
 select ' ' from dual;
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._13pkb.sql
 -- gera PACKAGE_BODY
 select ' ' from dual;
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._14pro.sql
 -- gera Procedure
 select ' ' from dual;
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._15trg.sql
 -- gera triggers
 select ' ' from dual;
 ...
 select 'CREATE OR REPLACE TRIGGER ',
  T.description,
  T.trigger_body
  from all_triggers T, tabelas&dblink A
 where A.aplicacao like '&dono' and A.tipo='TABELA' and
       T.table_owner=A.owner and T.table_name=A.tabela
 order by T.trigger_name;
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._16seq.sql
 -- gera sequences
 select ' ' from dual;
 ...
 select 'CREATE SEQUENCE '||sequence_name,
  ...
  ';'
  from all_sequences
 where sequence_owner||sequence_name in (select owner||tabela from tabelas&dblink A
                       where A.aplicacao like '&dono' and
                             A.tipo='SEQUENCE' )
 order by sequence_name;
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._17syn.sql
-- gera comando para criação do sinonimo
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._18role.sql
-- gera comando para criação das roles
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._19priv.sql
...
--
 ...
 ' to '||B.grantee||';'
 from dba_col_privs B, tabelas&dblink A
 where A.aplicacao='&dono' and
       B.owner = A.owner and B.table_name = A.tabela
 order by B.table_name,B.grantee,B.privilege;
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._21trunc.sql
-- gera comando para deletar os dados das tabelas 
 ...
 spool off
/
 spool C:\Sistemas\Sql\Infraero\Oracle\Scripts\script_ronei\&dono._22carga.sql
