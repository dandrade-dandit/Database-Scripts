SPO trcasplit_error.log;
SET DEF ON ECHO ON TERM OFF VER OFF SERVEROUT ON SIZE 1000000 NUMF "" SQLP SQL>;
SET SERVEROUT ON SIZE UNL;
REM $Header: 224270.1 trcasplit.sql 11.4.5.0 2012/11/21 carlos.sierra $
REM
...
REM   3. Execute script trcasplit.sql passing name of trace file that
REM      was generated by EVENT 10046 and some other (i.e. 10053).
REM
REM EXAMPLES
REM   # cd trca/run
REM   # sqlplus [apps user]
REM   SQL> start trcasplit.sql [name of your trace file]
REM   SQL> start trcasplit.sql largesql.trc  <== your trace file
REM   Notice that your trace file must end with ".trc".
REM
REM NOTES
REM   1. If you need the input directory be something else than UDUMP
REM      use then trca/utl/tacdiri1.sql connected as SYS.
REM   2. For possible errors see trcasplit_error.log generated under
...
PRO
PRO Value passed to trcasplit.sql:
PRO ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
PRO
SET TERM OFF;
EXEC &&tool_administer_schema..trca$i.trcanlzr(p_file_name => '&&input_filename.', p_analyze => 'NO', p_split => 'YES', x_tool_execution_id => :v_tool_execution_id);
...
PRO Review first trcasplit_error.log file for possible fatal errors.
...
SPO OFF;
HOS zip -mT trca_e&&tool_execution_id. trca_e&&tool_execution_id.* trcasplit_error.log
HOS zip -d trca_e&&tool_execution_id. trcasplit_error.log
