...
select to_char(timestamp,'MM-YYYY')||' Dept.:'||upper(substr(os_username,1,4))||' Qtd:'||count(*)
from dba_audit_session
where username = 'IFRMAXIMO'
group by to_char(timestamp,'MM-YYYY'),upper(substr(os_username,1,4));

select to_char(timestamp,'MM-YYYY')||' Dept.:'||upper(substr(os_username,1,4))||' Qtd:'||count(*)
from dba_audit_session
where username = 'IFRMAXIMO' and
to_char(timestamp,'MM') = 11
group by to_char(timestamp,'MM-YYYY'),upper(substr(os_username,1,4))
/

select to_char(timestamp,'DD-MM-YYYY HH:MI')||' Qtd:'||count(*)
from dba_audit_session
where username = 'IFRMAXIMO' and
to_char(timestamp,'MM') = 11
group by to_char(timestamp,'DD-MM-YYYY HH:MI')
/
